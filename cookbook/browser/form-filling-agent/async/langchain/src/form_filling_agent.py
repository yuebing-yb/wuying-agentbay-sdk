...
class BrowserIntegrationToolkit:
    ...
    async def initialize_browser(self, session):
        ...
        try:
            ...
            # Initialize browser
            init_result = await session.browser.initialize(BrowserOption())
            if not init_result:
...
async def create_langchain_form_filling_agent():
    """
    ...
    """
    # Create a real AgentBay browser session
    agent_bay, session = await create_real_session()
    ...
    await toolkit.initialize_browser(session)
