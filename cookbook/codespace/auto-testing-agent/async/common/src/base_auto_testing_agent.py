...
class BaseTestingAgent:
    ...
    async def upload_project_files(self):
        ...
        for file_info in project_files["files"]:
            ...
            if file_name.endswith('.py') and not file_name.startswith('test_'):
                ...
                write_result = await self.session.file_system.write_file(output_path, file_content)
    ...
    async def execute_test_file(self, filename: str, test_code: str) -> Dict:
        """
        ...
        write_result = await self.session.file_system.write_file(test_file_output_path, test_code)
        ...
        execution_result = await self.session.command.execute_command(f"python3 {test_file_output_path}")
