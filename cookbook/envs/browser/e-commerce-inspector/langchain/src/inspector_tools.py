...
async def take_and_save_screenshot(agent, base_url: str, out_dir: str, session=None, page=None) -> Optional[str]:
    """Take a screenshot and save it to the output directory."""
    try:
        host = urlparse(base_url).netloc
        
        # Take screenshot (viewport only, not full page)
        data_url = await agent.screenshot_async(full_page=False, quality=90)
