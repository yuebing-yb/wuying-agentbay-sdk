...
var (
	...
	maxCaptchaSolvingTimeout     = 60.0
	maxTakeoverTimeout           = 180.0
	captchaPauseEvent            = NewEvent()
	captchaResumeOrTakeoverEvent = NewEvent()
	mu                           sync.Mutex
)

func waitForCaptchaSolving(
	captchaPauseEvent *Event,
	captchaResumeOrTakeoverEvent *Event,
	captchaDetectTimeout float64,
) bool {
	fmt.Printf("Waiting for captcha pause event, timeout: %.1fs\n", captchaDetectTimeout)
	pauseDetected := captchaPauseEvent.Wait(time.Duration(captchaDetectTimeout * float64(time.Second)))

	if !pauseDetected {
		fmt.Println("No captcha pause event detected within timeout, proceeding next step")
		return true
	}

	mu.Lock()
	timeout := maxCaptchaSolvingTimeout
	mu.Unlock()

	fmt.Printf("Waiting for captcha resume event, timeout: %.1fs\n", timeout)
	resumeDetected := captchaResumeOrTakeoverEvent.Wait(time.Duration(timeout * float64(time.Second)))

	if !resumeDetected {
		fmt.Println("No captcha resume event detected within timeout, should takeover")
		return false
	}
	...
	if takeover {
		fmt.Println("Captcha solving failed, takeover event detected")
		return false
	}
	return true
}
...
func onBrowserCallback(msg *agentbaybrowser.BrowserNotifyMessage) {
	fmt.Printf("🔔 Received browser callback: %v, %v, %v, %v, %v, %v\n",
	...
	if msg.Type != nil && *msg.Type == "call-for-user" {
		...
		if action != nil && *action == "pause" {
			if waitTime, ok := extraParams["max_wait_time"].(float64); ok {
				mu.Lock()
				maxCaptchaSolvingTimeout = waitTime
				mu.Unlock()
				fmt.Printf("Captcha pause notification received, max wait time: %.1fs\n", waitTime)
			}
			captchaPauseEvent.Set()
		} else if action != nil && *action == "resume" {
			fmt.Println("Captcha resume notification received")
			captchaResumeOrTakeoverEvent.Set()
		} else if action != nil && *action == "takeover" {
			mu.Lock()
			shouldTakeover = true
...
func main() {
	...
	browserOption.SolveCaptchas = true
	...
	fmt.Println("🔑 Captcha triggered, waiting for solving...")

	captchaSolved := waitForCaptchaSolving(
		captchaPauseEvent,
	...
	if !captchaSolved {
		fmt.Println("⏰ Captcha solving timeout or should takeover...")
