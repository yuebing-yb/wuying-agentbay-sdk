...
func main() {
	...
	basicSync := agentbay.NewBasicContextSync(context.ID, "/data")
	...
	fmt.Printf("  - SyncMode: %s\n", advancedSync.Policy.SyncMode)
	fmt.Printf("  - WhiteList: Path=%s, ExcludePaths=%v\n",
		advancedSync.Policy.BWList.WhiteList.Path,
		advancedSync.Policy.BWList.WhiteList.ExcludePaths)
	...
	sessionParams.WithImageId("imgc-07eksy57bs4xb0ple")
	...
	builderSync := agentbay.NewContextSync(context.ID, "/workspace").
		WithSyncMode(agentbay.FullSync).
		WithUploadPolicy(&agentbay.UploadPolicy{
			AutoUpload:     true,
			UploadStrategy: agentbay.ImmediateUpload,
		}).
		WithDownloadPolicy(&agentbay.DownloadPolicy{
			AutoDownload:     true,
			DownloadStrategy: agentbay.DownloadOnDemand,
		}).
		WithWhiteList("/workspace/src", []string{"/workspace/src/node_modules"})

	fmt.Printf("Builder sync - ContextID: %s, Path: %s, SyncMode: %s\n",
		builderSync.ContextID, builderSync.Path, builderSync.Policy.SyncMode)
