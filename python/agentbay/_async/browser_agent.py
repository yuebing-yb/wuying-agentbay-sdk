    ...
    async def extract(
        ...
        try:
            ...
            if isinstance(options, str):
                return await self._execute_simple_extract(options, context_id, page_id)
            
            return await self._execute_extract(options, context_id, page_id)
        except Exception as e:
            raise BrowserError(f"Failed to extract: {e}")

    async def _execute_simple_extract(
        self,
        instruction: str,
        context_id: int,
        page_id: Optional[str],
        ...
        response = await self._call_mcp_tool_timeout("page_use_extract", args)
        if not response.success:
