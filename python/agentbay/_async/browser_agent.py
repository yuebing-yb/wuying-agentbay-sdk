...
from ..exceptions import BrowserError, AgentBayError
...
class AsyncBrowserAgent(AsyncBaseService):
    ...
    async def close(self) -> bool:
        ...
        """
        try:
            response = await self._call_mcp_tool_timeout("page_use_close_session", args={})
            if response.success:
            ...
            if isinstance(response_data, dict):
                ...
                if not task_id:
                    ...
                    if "steps" in response_data or "is_done" in response_data:
                        ...
                        _logger.info(f"Task {task_name} completed immediately. Success: {success}")
                        return ActResult(success=success, message=task_status, action=task_name)
                ...
                _logger.info(f"Task {task_name} completed immediately with steps: {task_status}")
                return ActResult(success=True, message=task_status, action=task_name)
            else:
                raise BrowserError(f"Unexpected response format: {type(response_data)}, data: {response_data}")
        except (KeyError, TypeError, IndexError) as e:
            raise BrowserError(f"Failed to parse task_id from response: {e}, data: {response.data}")
        ...
        while max_retries > 0:
            ...
            if result.success and result.data:
                ...
                if isinstance(data, list):
                    ...
                    _logger.warning(f"Received list data for act result, skipping this iteration")
                    max_retries -= 1
                    continue
            ...
            if isinstance(response_data, dict):
                ...
                if not task_id:
                    ...
                    try:
                        ...
                        raise BrowserError(f"No task_id and failed to validate response as result: {e}, data: {response_data}")
            elif isinstance(response_data, list):
                # Unexpected list response
                raise BrowserError(f"Unexpected list response for extract: {response_data}")
            else:
                raise BrowserError(f"Unexpected response format for extract: {type(response_data)}")
        except (KeyError, TypeError, IndexError) as e:
            raise BrowserError(f"Failed to parse extract response: {e}, data: {response.data}")
