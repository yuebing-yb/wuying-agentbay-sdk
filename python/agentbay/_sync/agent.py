...
SyncAgentEventCallback = Optional[Callable[[AgentEvent], Union[Awaitable[str], str]]]
...
class TaskExecution:
    ...
    def _wait_ws(self, timeout: int) -> ExecutionResult:
        ctx = self._context
        try:
            end_data = self._ws_handle.wait_end_with_timeout(timeout)
        except TimeoutError:
            try:
                self._ws_handle.cancel()
            except Exception:
                pass
    ...
    def _wait_polling(self, timeout: int) -> ExecutionResult:
        agent = self._agent
        ...
        while tried_time < max_poll_attempts:
            query = agent.get_task_status(task_id)
            if query.stream:
                last_query = query
        ...
        try:
            terminate_result = agent.terminate_task(task_id)
        ...
        while terminate_tried < 30:
            try:
                status_query = agent.get_task_status(task_id)
...
class Agent(BaseService):
    ...
    class _BaseTaskAgent(BaseService):
        ...
        def _start_task_stream_ws(
            ...
            def _handle_call_for_user(event: AgentEvent) -> None:
                response = ""
                if on_call_for_user:
                    try:
        ...
        def _execute_task_stream_ws(
            ...
            ws_handle, ctx = self._start_task_stream_ws(
