...
def take_and_save_screenshot(agent, base_url: str, out_dir: str):
    base64_screenshot = agent.screenshot()
...
def process_site(agent, url: str, out_dir: str = "/tmp") -> None:
    host = domain_of(url)
    agent.navigate(url)
...
def run(
    ...
    for url in SITES:
        try:
            process_site(agent, url, out_dir=f"./results_{date_str}")
        except Exception as e:
