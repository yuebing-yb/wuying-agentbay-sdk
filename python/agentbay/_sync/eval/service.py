...
def identify_key_points(task, model):
...
- **Key Points**: A numbered list of the explicit key points for completing this task, one per line, without explanations or additional details."""
    ...
    response = asyncio.to_thread(model.invoke, messages)
    return response.content


def judge_image(task, image_path, key_points, model):
...
def Online_Mind2Web_eval(task, last_actions, images_path, model, score_threshold):
...
{thoughts}"""
    ...
    tasks = [
        judge_image(task, image_path, key_points, model) for image_path in images_path
    ]
    image_responses = asyncio.gather(*tasks)
...
def Online_Mind2Web_eval_with_retry(
    task, last_actions, images_path, model, score_threshold, max_retries=3
):
    """
    ...
    for attempt in range(max_retries):
        try:
            return Online_Mind2Web_eval(
                task, last_actions, images_path, model, score_threshold
            )
        except Exception as e:
            if attempt == max_retries - 1:  # Last attempt
                _logger.error(
                    f"Failed to evaluate after {max_retries} attempts. Error: {type(e).__name__}: {str(e)}"
                )
                raise
            _logger.warning(
                f"Attempt {attempt + 1} failed. Retrying... Error: {type(e).__name__}: {str(e)}"
            )
...
if not SERPER_API_KEY:
    _logger.warning(
        "SERPER_API_KEY is not set. Search functionality will not be available."
    )
...
def create_controller_with_serp_search():
    ...
    def search_web(query: str):
        ...
        if not SERPER_API_KEY:
            return ActionResult(
                extracted_content="Search unavailable: SERPER_API_KEY not configured",
        ...
        try:
            ...
            serp_data = {
                ...
                if k not in ["searchParameters", "credits"]
            }

            # Log the search data for debugging
            _logger.debug(
                f"SERP search for '{query}': {json.dumps(serp_data, indent=2)}"
            )
            ...
            return ActionResult(
                extracted_content=serp_data_str, include_in_memory=False
            )

        except Exception as e:
            _logger.error(f"Error in SERP search: {type(e).__name__}: {e}")
            return ActionResult(
                extracted_content=f"Search error: {str(e)}", include_in_memory=False
            )
...
def get_llm(model_name: str):
    ...
    if model_name not in SUPPORTED_MODELS:
        raise ValueError(
            f"Unsupported model: {model_name}. Supported models are: {list(SUPPORTED_MODELS.keys())}"
        )
...
def clean_action_dict(action_dict: dict) -> dict:
    return {
        k: clean_action_dict(v) if isinstance(v, dict) else v
        for k, v in action_dict.items()
        if v is not None
    }


def reformat_agent_history(
    ...
    for step_num, history_item in enumerate(agent_history.history):
        ...
        if history_item.state and history_item.state.screenshot:
            ...
            with anyio.open_file(screenshot_path, "wb") as f:
        ...
        if history_item.result:
            for result in history_item.result:
                ...
                if (
                    ...
                    and result.extracted_content != "None"
        ...
        if history_item.model_output:
            ...
            if "action" in model_output:
                ...
                model_output["action"] = [
                    clean_action_dict(action) for action in model_output["action"]
                ]

        step_metadata = (
            history_item.metadata.model_dump() if history_item.metadata else {}
        )
        step_info = {
            "step_number": step_num,
            "model_output": model_output,
            "result": (
                [r.model_dump() for r in history_item.result]
                if history_item.result
                else None
            ),
            "state": {
        ...
        if first_step and last_step:
            ...
            if start_time and end_time:
                ...
                try:
                    ...
                    _logger.warning(
                        f"Could not calculate task duration due to invalid timestamp format: {e}"
                    )
    ...
    with anyio.open_file(results_path, "w") as f:
...
def judge_task_result(
    ...
    try:
        ...
        try:
            ...
            judgement_msg = asyncio.to_thread(model.invoke, messages)
            judgement = judgement_msg.content

            if (
                "success" in judgement.lower().split("status:")[1]
            ):  # This is the official criteria for success
                evaluation = {
                    "task_id": task_folder.name,
                    "judgement": judgement,
                    "success": True,
    ...
    for folder in task_folders:
        ...
        if result_file.exists():
            try:
                ...
                _logger.error(
                    f"Error reading result file {result_file}: {type(e).__name__}: {e}"
                )
...
class TaskResult:
    ...
    def has_execution_data(self) -> bool:
        ...
        return (
            Stage.LOAD_EXISTING in self.completed_stages
            or Stage.FORMAT_HISTORY in self.completed_stages
        )
    ...
    def _auto_update_payload(self):
            ...
            self.server_payload.update(
                {
                    ...
                    "finalResultResponse": existing_data.get(
                        "final_result_response", "None"
                    ),
                    "selfReportCompleted": existing_data.get(
                        "self_report_completed", False
                    ),
            ...
            self.server_payload.update(
                {
                    ...
                    "finalResultResponse": formatted_data.get(
                        "final_result_response", "None"
                    ),
                    "selfReportCompleted": formatted_data.get(
                        "self_report_completed", False
                    ),
                    "selfReportSuccess": formatted_data.get(
                        "self_report_success", None
                    ),
        ...
        if Stage.EVALUATE in self.completed_stages:
            ...
            self.server_payload.update(
                {
                    "onlineMind2WebEvaluationJudgement": (
                        judgement if judgement is not None else "None"
                    ),
        ...
        for stage, error in self.failed_stages.items():
            if stage in {Stage.SETUP_BROWSER, Stage.RUN_AGENT}:
                ...
                if error.error_type == "timeout":
                    self.server_payload["browserCrashReason"] = (
                        f"{stage.value} timed out: {error.message}"
                    )
                elif error.error_type == "cancelled":
                    self.server_payload["browserCrashReason"] = (
                        f"{stage.value} was cancelled: {error.message}"
                    )
                else:
                    self.server_payload["browserCrashReason"] = (
                        f"{stage.value} failed: {error.message}"
                    )

            # Update evaluation failures
            elif stage == Stage.EVALUATE:
                if error.error_type == "timeout":
                    self.server_payload["onlineMind2WebEvaluationJudgement"] = (
                        "Evaluation Timed Out"
                    )
                    self.server_payload["onlineMind2WebEvaluationError"] = (
                        "Evaluation process timed out"
                    )
                elif error.error_type == "cancelled":
                    self.server_payload["onlineMind2WebEvaluationJudgement"] = (
                    ...
                    self.server_payload["onlineMind2WebEvaluationJudgement"] = (
                        "Evaluation Process Error"
                    )
                    self.server_payload["onlineMind2WebEvaluationError"] = (
                        f"Evaluation Error: {error.message}"
                    )
    ...
    def get_local_status(self) -> dict:
        ...
        success = self.execution_succeeded() and (
            ...
            or not self.has_execution_data()
            or Stage.EVALUATE in self.failed_stages
        )
        return {
            "task_id": self.task_id,
            "success": success and not self.local_error,
            "error": self.local_error,
        }


def run_stage(stage: Stage, stage_func, timeout: int | None = None):
    """Generic stage runner with timeout"""
    if timeout:
        return asyncio.wait_for(stage_func(), timeout)
    return stage_func()


def load_existing_result(task_folder: Path) -> dict:
    ...
    with anyio.open_file(result_file) as f:
        existing_result = json.loads(f.read())
...
def setup_browser_session(task: Task, headless: bool) -> BrowserSession:
    """Setup browser session for the task"""
    _logger.debug(
        f"Browser setup: Creating unique user data directory for task {task.task_id}"
    )
    ...
    if task.website:
        _logger.debug(
            f"Browser setup: Navigating to {task.website} for task {task.task_id}"
        )
        browser_session.navigate(task.website)

    _logger.debug(f"Browser setup: Setup completed for task {task.task_id}")
    return browser_session


def run_agent_with_browser(
    browser_session: BrowserSession,
    task: Task,
    llm: BaseChatModel,
    max_steps: int,
    ...
    if enable_memory:
        memory_config = MemoryConfig(
            agent_id=f"eval_agent_{task.task_id}",
...
def evaluate_task_result(eval_model: BaseChatModel, task_folder: Path) -> dict:
    """Evaluate the task result"""
    return judge_task_result(eval_model, task_folder, score_threshold=3)
...
def cleanup_browser_safe(browser_session: BrowserSession):
    """Safe browser cleanup with timeout"""
    try:
        _logger.debug("Browser cleanup: Starting close operation for session")
        asyncio.wait_for(browser_session.close(), timeout=30)
...
def run_task_with_semaphore(
    ...
    _logger.info(
        f"Task {task.task_id}: Waiting to acquire semaphore (current value: ~{semaphore_runs._value})"
    )
    with semaphore_runs:
        _logger.info(
            f"Task {task.task_id}: Semaphore acquired (remaining slots: ~{semaphore_runs._value})"
        )
        task_result = None
        ...
        try:
            ...
            task_result = TaskResult(
                task.task_id, run_id, task.confirmed_task, task, max_steps_per_task
            )
            ...
            try:
                ...
                try:
                    existing_data = run_stage(
                        Stage.LOAD_EXISTING, lambda: load_existing_result(task_folder)
                    )
                    ...
                    if existing_data.get("has_evaluation"):
                        task_result.stage_completed(
                            Stage.EVALUATE, existing_data["evaluation_data"]
                        )

                    _logger.info(
                        f"Task {task.task_id}: Successfully loaded existing result. Skipping execution."
                    )

                except Exception:
                    # No existing result, need to execute full pipeline
                    _logger.info(
                        f"Task {task.task_id}: No existing result found. Starting execution pipeline."
                    )
                    ...
                    try:
                        ...
                        browser_session = run_stage(
                            Stage.SETUP_BROWSER,
                            lambda: setup_browser_session(task, headless),
                            timeout=120,
                        )
                        task_result.stage_completed(Stage.SETUP_BROWSER)
                        _logger.info(
                            f"Task {task.task_id}: Browser session started successfully."
                        )
                    except Exception as e:
                        error = StageError(Stage.SETUP_BROWSER, "exception", str(e))
                        task_result.stage_failed(Stage.SETUP_BROWSER, error)
                        _logger.error(
                            f"Task {task.task_id}: Browser setup failed: {str(e)}"
                        )
                    ...
                    if browser_session:  # Only run agent if browser setup succeeded
                        try:
                            ...
                            _logger.error(
                                f"Task {task.task_id}: Agent run failed: {str(e)}"
                            )
                            # Continue to server save instead of early return

                    # Stage 4: Format history
                    if (
                        agent_history is not None
                    ):  # Only format if agent ran successfully
                        try:
                            _logger.info(
                                f"Task {task.task_id}: History formatting starting."
                            )
                            formatted_data = run_stage(
                            ...
                            task_result.stage_completed(
                                Stage.FORMAT_HISTORY, formatted_data
                            )
                            _logger.info(
                                f"Task {task.task_id}: Agent history formatted."
                            )
                        except Exception as e:
                            error = StageError(
                                Stage.FORMAT_HISTORY, "exception", str(e)
                            )
                            task_result.stage_failed(Stage.FORMAT_HISTORY, error)
                            _logger.error(
                                f"Task {task.task_id}: History formatting failed: {str(e)}"
                            )
                            # Continue to server save instead of early return

                # Stage 5: Evaluate (if we have execution data and no existing evaluation)
                if (
                    task_result.has_execution_data()
                    and Stage.EVALUATE not in task_result.completed_stages
                ):
                    try:
                        _logger.info(f"Task {task.task_id}: Evaluation starting.")
                        evaluation = run_stage(
                            Stage.EVALUATE,
                            lambda: evaluate_task_result(eval_model, task_folder),
                            timeout=300,
                        )
                        ...
                        _logger.error(
                            f"Task {task.task_id}: Evaluation failed: {str(e)}"
                        )
                ...
                try:
                    ...
                    _logger.info(
                        f"Task {task.task_id}: Successfully saved result to server."
                    )
                except Exception as e:
                ...
                try:
                    _logger.info(
                        f"Task {task.task_id}: Attempting server save after timeout."
                    )
                    run_stage(
                        Stage.SAVE_SERVER,
                        lambda: asyncio.to_thread(
                    ...
                    _logger.error(
                        f"Task {task.task_id}: Emergency server save after timeout failed: {str(save_e)}"
                    )
                ...
                try:
                    _logger.info(
                        f"Task {task.task_id}: Attempting server save after cancellation."
                    )
                    run_stage(
                        Stage.SAVE_SERVER,
                        lambda: asyncio.to_thread(
                    ...
                    task_result.mark_server_save_failed(str(save_e))
                    _logger.error(
                        f"Task {task.task_id}: Emergency server save after cancellation failed: {str(save_e)}"
                    )

            except Exception as e:
                task_result.mark_critical_error(str(e))
                _logger.critical(
                    f"Task {task.task_id}: Critical error: {str(e)}", exc_info=True
                )

                # Attempt to save result even if critical error occurred
                try:
                    _logger.info(
                        f"Task {task.task_id}: Attempting server save after critical error."
                    )
                    run_stage(
                        Stage.SAVE_SERVER,
                        lambda: asyncio.to_thread(
                    ...
                    task_result.mark_server_save_failed(str(save_e))
                    _logger.error(
                        f"Task {task.task_id}: Emergency server save after critical error failed: {str(save_e)}"
                    )

        except Exception as init_error:
            # Handle catastrophic initialization errors
            _logger.critical(
                f"Task {task.task_id}: Catastrophic initialization error: {str(init_error)}",
                exc_info=True,
            )
            if task_result is None:
                ...
                try:
                    ...
                    task_result.mark_critical_error(
                        f"Initialization failed: {str(init_error)}"
                    )
                except Exception as result_error:
                    _logger.critical(
                        f"Task {task.task_id}: Cannot create TaskResult: {str(result_error)}"
                    )
            ...
            try:
                ...
                _logger.error(
                    f"Task {task.task_id}: Emergency server save after initialization error failed: {str(save_e)}"
                )
            ...
            if browser_session:
                ...
                cleanup_browser_safe(browser_session)
                _logger.info(f"Task {task.task_id}: Browser cleanup completed")
            else:
                _logger.info(f"Task {task.task_id}: No browser to cleanup")

        _logger.info(
            f"Task {task.task_id}: About to release semaphore (remaining slots: ~{semaphore_runs._value})"
        )
        return (
...
def run_multiple_tasks(
    ...
    _logger.info(
        f"Starting {len(tasks_to_run)} tasks with parallel limit of {max_parallel_runs}"
    )

    # Run all tasks in parallel with additional parameters
    task_results = asyncio.gather(
        *(
    ...
    for i, result in enumerate(task_results):
        if isinstance(result, Exception):
            _logger.error(
                f"Task {i} failed with exception: {type(result).__name__}: {result}"
            )
            processed_results.append(
                {"task_id": f"task_{i}", "success": False, "error": str(result)}
            )
            failed_tasks += 1
        else:
    ...
    _logger.info(
        f"All {len(tasks_to_run)} tasks completed. Success: {successful_tasks}, Failed: {failed_tasks}"
    )
    ...
    try:
        ...
        if response.status_code == 200:
            try:
                data = response.json()
                _logger.info(
                    f"Successfully fetched test case data for '{test_case_name}'."
                )
                ...
                if isinstance(data, list):
                    ...
                    _logger.error(
                        f"Error: Fetched data is not a list. Type: {type(data)}"
                    )
            ...
            _logger.error(
                f"Error: Failed to fetch test case '{test_case_name}'. Status: {response.status_code}"
            )
        ...
        _logger.error(
            f"Error during request to fetch test case: {type(e).__name__}: {e}"
        )
        return None
...
def get_git_info():
    ...
    try:
        ...
        _logger.warning(
            f"Could not retrieve git info: {type(e).__name__}: {e}. Using defaults."
        )
        return {
            "branch": "unknown",
            "hash": "unknown",
        ...
        if response.status_code == 200:
            try:
                ...
                if run_id:
                    ...
                    _logger.error(
                        "Error: 'runId' not found in successful startRun response."
                    )
                    _logger.error(f"Raw response: {response.text}")
                    return None
            except json.JSONDecodeError:
...
def save_task_result_to_server(convex_url: str, secret_key: str, result_details: dict):
    ...
    if not convex_url:
        _logger.error(
            "Error: EVALUATION_TOOL_URL environment variable not set for saving task result."
        )
        return False

    if not secret_key:
        _logger.error(
            "Error: EVALUATION_TOOL_SECRET_KEY environment variable not set for saving task result."
        )
        return False
    ...
    if "runId" not in result_details or not result_details["runId"]:
        _logger.error(
            "Error: 'runId' is missing or empty in result_details for saveTaskResult."
        )
        return False
    ...
    _logger.debug(
        f"Result details payload: {json.dumps(result_details, indent=2)}"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run and evaluate browser automation tasks"
    )
    parser.add_argument(
        "--parallel-runs", type=int, default=3, help="Number of parallel tasks to run"
    )
    parser.add_argument(
        "--max-steps", type=int, default=25, help="Maximum steps per task"
    )
    ...
    parser.add_argument(
        "--evaluate-only",
        action="store_true",
        help="Only evaluate existing results without running new tasks",
    )
    parser.add_argument(
        "--model",
        type=str,
    ...
    parser.add_argument(
        "--no-vision",
        action="store_true",
        help="Disable vision capabilities in the agent",
    )
    parser.add_argument(
        "--fresh-start",
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="User message to include in the run",
    )
    parser.add_argument(
        "--eval-group",
        type=str,
        default="",
        help="Evaluation group to include in the run",
    )
    parser.add_argument(
        "--developer-id",
        type=str,
        default=None,
        help="Name of the developer starting the run",
    )
    parser.add_argument(
        "--use-serp",
        action="store_true",
        help="Use SERP search instead of Google search",
    )
    parser.add_argument(
        "--enable-memory",
        action="store_true",
        help="Enable mem0 memory system for agents",
    )
    parser.add_argument(
        "--memory-interval",
        type=int,
        default=10,
        help="Memory creation interval (default: 10 steps)",
    )
    parser.add_argument(
        "--max-actions-per-step",
        type=int,
        default=10,
        help="Maximum number of actions per step (default: 10)",
    )
    parser.add_argument(
        "--validate-output",
        action="store_true",
        help="Enable output validation using LLM",
    )
    parser.add_argument(
        "--planner-model",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Run planner every N steps (default: 1)",
    )
    ...
    if args.evaluate_only:
        ...
        _logger.info(
            f'Evaluation complete. Success rate: {summary["success_rate"]:.2%}'
        )
        _logger.info(f'Average score: {summary["average_score"]:.2f}')
        ...
        if args.fresh_start:
            ...
            if results_dir_path.exists():
                try:
                    ...
                    _logger.error(
                        f"Error removing directory {results_dir_path}: {type(e).__name__}: {e}"
                    )
            ...
            try:
                ...
                _logger.error(
                    f"Error creating directory {results_dir_path}: {type(e).__name__}: {e}"
                )
                # exit(1) # Uncomment to exit on error
        else:
            _logger.info(
                "--fresh-start is False. Existing trajectories in saved_trajectories will be kept."
            )
        # -------------------------------------------------
        ...
        if not CONVEX_URL or not SECRET_KEY:
            _logger.error(
                "Error: EVALUATION_TOOL_URL or EVALUATION_TOOL_SECRET_KEY environment variables not set."
            )
            exit(1)  # Exit if config is missing

        _logger.info(f"Attempting to fetch task list '{TEST_CASE_NAME}' from server...")
        fetched_task_data = fetch_tasks_from_server(
            CONVEX_URL, SECRET_KEY, TEST_CASE_NAME
        )
        ...
        try:
            ...
            _logger.error(
                f'First item in fetched data: {fetched_task_data[0] if fetched_task_data else "None"}'
            )
            exit(1)
        # -----------------------------
        ...
        run_data = {
            ...
            "totalTasks": (
                len(tasks) - args.start if args.end is None else args.end - args.start
            ),
        ...
        _logger.info(
            f"Successfully obtained run ID: {run_id}. Proceeding with tasks..."
        )
        ...
        if args.use_serp:
            if SERPER_API_KEY:
                _logger.info(
                    "🔍 Using SERP search (Serper API) instead of Google search"
                )
            else:
                _logger.warning(
                    "⚠️ --use-serp flag provided but SERPER_API_KEY not set. Search will fail!"
                )
        else:
        ...
        if args.enable_memory:
            _logger.info(
                f"🧠 Memory enabled: mem0 system with interval={args.memory_interval} steps"
            )
        else:
        ...
        if args.planner_model:
            _logger.info(
                f"🗺️ Planner enabled: {args.planner_model} (interval={args.planner_interval} steps)"
            )
        else:
        ...
        try:
            ...
            _logger.error(
                f"Failed to instantiate agent LLM ({args.model}): {type(e).__name__}: {e}",
                exc_info=True,
            )
            exit(1)
        ...
        try:
            ...
            _logger.info(
                f"Evaluation LLM ({args.eval_model}) instantiated successfully."
            )
        except Exception as e:
            _logger.error(
        ...
        if args.planner_model:
            ...
            try:
                ...
                _logger.info(
                    f"Planner LLM ({args.planner_model}) instantiated successfully."
                )
            except Exception as e:
                _logger.error(
