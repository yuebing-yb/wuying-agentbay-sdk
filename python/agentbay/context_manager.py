...
class ContextStatusData:
    def __init__(self, context_id: str = "", path: str = "", error_message: str = "",
                 status: str = "", start_time: int = 0, finish_time: int = 0, task_type: str = ""):
...
class ContextInfoResult(ApiResponse):
    def __init__(self, request_id: str = "", context_status_data: List[ContextStatusData] = None):
                    ...
                    for item in status_items:
                        if item.get("type") == "data":
                            ...
                            for data_item in data_items:
                                context_status_data.append(ContextStatusData.from_dict(data_item))
                except json.JSONDecodeError as e:
                    print(f"Error parsing context status: {e}")
                except Exception as e:
                    print(f"Unexpected error parsing context status: {e}")
        
        return ContextInfoResult(request_id=request_id, context_status_data=context_status_data)
