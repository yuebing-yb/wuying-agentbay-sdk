...
#### SyncAgentEventCallback

```python
SyncAgentEventCallback = Optional[Callable[[AgentEvent], Union[Awaitable[str],
                      ...
```
...
```python
def wait(timeout: int = 300) -> ExecutionResult
```
...
class WeatherSchema(BaseModel):
  city:str
  weather: str
result = session.agent.browser.execute_task_and_wait(task="Query the weather in Shanghai",timeout=60, use_vision=False, output_schema=WeatherSchema, full_page_screenshot=True)
...
def execute_task(
        ...
        on_call_for_user: SyncAgentEventCallback = None) -> TaskExecution
```
...
execution = session.agent.mobile.execute_task(
  "Open WeChat app",
  max_steps=100,
  on_reasoning=lambda e: print(e.content, end="", flush=True),
)
result = execution.wait(timeout=180)
print(f"Task result: {result.task_result}")

# Non-blocking without streaming (poll with wait)
execution = session.agent.mobile.execute_task("Open Settings")
result = execution.wait(timeout=60)
...
def execute_task_and_wait(
        ...
        on_tool_result: AgentEventCallback = None,
        on_error: AgentEventCallback = None,
        on_call_for_user: SyncAgentEventCallback = None) -> ExecutionResult
```
