...
async def main():
    ...
    try:
        ...
        await operator.navigate("https://www.aliyun.com")
        await operator.act(ActOptions(action="搜索框输入'AgentBay帮助文档'并回车"))
        await operator.act(ActOptions(action="点击搜索结果中的第一项"))
        await operator.act(ActOptions(action="点击'帮助文档'"))
        await operator.act(ActOptions(action="滚动页面到底部"))
