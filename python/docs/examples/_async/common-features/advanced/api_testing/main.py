...
async def test_post_request(session, url: str, data: Dict[str, Any]) -> Dict[str, Any]:
    """Test a POST request."""
...
async def test_put_request(session, url: str, data: Dict[str, Any]) -> Dict[str, Any]:
    """Test a PUT request."""
    print(f"\n🔄 Testing PUT request: {url}")
...
async def test_delete_request(session, url: str) -> Dict[str, Any]:
    """Test a DELETE request."""
    print(f"\n🗑️  Testing DELETE request: {url}")
...
async def test_api_with_auth(session, url: str, token: str) -> Dict[str, Any]:
    """Test API with authentication."""
    print(f"\n🔐 Testing authenticated request: {url}")
...
async def validate_json_response(response_body: str, expected_fields: list) -> bool:
...
async def perform_load_test(session, url: str, num_requests: int = 10):
    """Perform simple load testing."""
    print(f"\n⚡ Performing load test: {num_requests} requests to {url}")
...
async def main():
    ...
    try:
        ...
        get_result = await test_get_request(session, f"{test_api_url}/posts/1")
        
        if get_result.get("success"):
            await validate_json_response(
                get_result["body"],
        ...
        await test_post_request(session, f"{test_api_url}/posts", post_data)
        ...
        await test_put_request(session, f"{test_api_url}/posts/1", put_data)
        ...
        await test_delete_request(session, f"{test_api_url}/posts/1")
        ...
        await perform_load_test(session, f"{test_api_url}/posts/1", num_requests=5)
        
        print("\n✅ API testing examples completed")
