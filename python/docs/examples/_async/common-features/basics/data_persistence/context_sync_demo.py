...
async def context_sync_with_callback_demo(agent_bay):
    ...
    context_result = await agent_bay.context.get("sync-callback-demo", create=True)
    ...
    sync_result = await session.context.sync(callback=sync_callback)
...
async def context_sync_demo(agent_bay):
    ...
    # Create context and session
    print("\n📦 Creating context and session...")
    context_result = await agent_bay.context.get("sync-await-demo", create=True)
    if not context_result.success:
        print(f"❌ Context creation failed: {context_result.error_message}")
        return
