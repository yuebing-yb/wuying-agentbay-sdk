...
async def example_1_default_recycle_policy():
    ...
    # Create a context
    context_result = await agent_bay.context.get("default-recycle-demo", create=True)
    ...
    # Write some data
    await session.command.execute_command("echo 'Default policy - data kept forever' > /tmp/default_data/test.txt")
...
async def example_2_one_day_lifecycle():
    ...
    # Create a context
    context_result = await agent_bay.context.get("one-day-recycle-demo", create=True)
    if not context_result.success:
        print(f"❌ Failed to create context: {context_result.error_message}")
        return
    ...
    # Write some data
    await session.command.execute_command("mkdir -p /tmp/oneday_data")
    await session.command.execute_command("echo 'This data will be deleted after 1 day' > /tmp/oneday_data/test.txt")
...
async def example_3_specific_paths():
    ...
    # Create a context
    context_result = await agent_bay.context.get("specific-path-demo", create=True)
    if not context_result.success:
        print(f"❌ Failed to create context: {context_result.error_message}")
        return
