...
async def main():
    ...
    try:
        ...
        result = await fs.write_file(test_file_path, append_content, "append")
        ...
        result = await fs.edit_file(test_file_path, edits)
        ...
        result = await fs.move_file(source_path, dest_path)
        ...
        result = await fs.read_file(dest_path)
        if result.success:
        ...
        result = await fs.search_files("/tmp/test_directory", "*report*")
        if result.success:
        ...
        result = await fs.search_files("/tmp/test_directory", "*.txt")
        if result.success:
            search_results = result.matches
        ...
        result = await fs.search_files("/tmp/test_directory", "*2025*")
        if result.success:
            search_results = result.matches
        ...
        result = await fs.read_multiple_files(file_paths)
        if result.success:
