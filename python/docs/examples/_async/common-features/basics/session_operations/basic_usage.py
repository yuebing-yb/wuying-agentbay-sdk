...
async def main():
    ...
    try:
        ...
        # Execute a command
        print("\nExecuting a command...")
        cmd_result = await session.command.execute_command("echo 'Hello Async World'")
        print(f"Command execution success: {cmd_result.success}")
        if cmd_result.success:
            print(f"Command output: {cmd_result.output}")
        else:
            print(f"Command error: {cmd_result.error_message}")
        ...
        # Get session link (Async)
        print("\nGetting session link...")
        link_result = await session.get_link()
        ...
        try:
            link_result_invalid = await session.get_link(None, 8080)
        ...
        print("\n=== Testing Linux Session with get_link parameters (Async) ===")
        linux_params = CreateSessionParams(
            image_id="linux_latest",
        )
        print("Creating a linux_latest session...")
        linux_session_result = await agent_bay.create(linux_params)
        ...
        try:
            link_result_with_params = await linux_session.get_link("https", 30199)
        ...
        try:
            link_result_invalid_params = await linux_session.get_link("https", 443)
        ...
        if linux_session:
            try:
                linux_delete_result = await agent_bay.delete(linux_session)
                linux_session = None
