...
async def main():
    ...
    try:
        ...
        apps_result = await session.computer.get_installed_apps(
            start_menu=True,
            desktop=False,
        ...
        start_result = await session.computer.start_app(notepad_cmd)
        ...
        visible_result = await session.computer.list_visible_apps()
        ...
        if main_process:
            ...
            stop_result = await session.computer.stop_app_by_pid(main_process.pid)
            ...
            if stop_result.success:
                ...
                stop_result = await session.computer.stop_app_by_pname(main_process.pname)
