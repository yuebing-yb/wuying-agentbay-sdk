...
session1: AsyncSession = None
session2: AsyncSession = None
mobile_sim_context_id: str = None


async def run_on_first_mobile_session(agent_bay: AsyncAgentBay):
    global session1
...
async def run_on_second_mobile_session(agent_bay: AsyncAgentBay):
    global session2
    global mobile_sim_context_id
    try:
        # Creating mobile simulate service and set simulate params
...
async def main():
    ...
    await run_on_first_mobile_session(agent_bay)
    ...
    await run_on_second_mobile_session(agent_bay)
