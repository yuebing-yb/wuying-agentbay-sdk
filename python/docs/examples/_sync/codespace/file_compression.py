...
def main():
    ...
    try:
        ...
        session.command.execute_command("mkdir -p /tmp/testdata")

        # Create multiple test files
        for i in range(5):
            content = f"This is test file {i+1}\n" * 100
            session.file_system.write_file(f"/tmp/testdata/file{i+1}.txt", content)

        print("Test files created in /tmp/testdata/")

        # List files and check sizes
        print("\n2. Listing test files...")
        result = session.command.execute_command("ls -lh /tmp/testdata/")
        print(f"Test files:\n{result.output}")
        ...
        result = session.command.execute_command("ls -lh /tmp/testdata.tar")
        ...
        result = session.command.execute_command("ls -lh /tmp/testdata.tar.gz")
        ...
        result = session.command.execute_command("ls -lh /tmp/testdata.zip")
        ...
        # Verify extraction
        result = session.command.execute_command("ls -lh /tmp/extract_tar/testdata/")
