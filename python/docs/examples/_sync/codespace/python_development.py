...
def main():
    ...
    try:
        ...
        result = session.command.execute_command("python3 --version")
...
if __name__ == "__main__":
    main()
"""
        session.file_system.write_file("/tmp/hello.py", script_content)
        print("Script created: /tmp/hello.py")

        # Run the Python script
        print("\n3. Running the Python script...")
        result = session.command.execute_command("python3 /tmp/hello.py")
        print(f"Script output:\n{result.output}")

        # Install a package with pip
        print("\n4. Installing requests package...")
        result = session.command.execute_command("pip3 install requests --quiet")
        if result.success:
...
if __name__ == "__main__":
    main()
"""
        session.file_system.write_file("/tmp/test_requests.py", requests_script)
        print("Script created: /tmp/test_requests.py")

        # Run the requests script
        print("\n6. Running the requests script...")
        result = session.command.execute_command("python3 /tmp/test_requests.py")
        print(f"Script output:\n{result.output}")

        # List installed packages
        print("\n7. Listing installed packages...")
        result = session.command.execute_command("pip3 list | head -20")
        print(f"Installed packages:\n{result.output}")

        # Create a requirements.txt
        print("\n8. Creating requirements.txt...")
        requirements = "requests>=2.28.0\n"
        session.file_system.write_file("/tmp/requirements.txt", requirements)
        print("requirements.txt created")

        # Verify the file
        content = session.file_system.read_file("/tmp/requirements.txt")
