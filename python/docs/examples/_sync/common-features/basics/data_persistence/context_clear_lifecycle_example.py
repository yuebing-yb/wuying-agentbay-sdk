...
def upload_file_to_context(
    ...
    try:
        # Get upload URL
        url_result = agentbay.context.get_file_upload_url(context_id, file_path)
...
def data_lifecycle_demonstration():
    ...
    try:
        ...
        create_result = agentbay.context.create(context_name)
        ...
        for file_path, content, file_type in files_to_upload:
            success = upload_file_to_context(agentbay, context_id, file_path, content)
            if success:
                print(f"  ✓ Uploaded: {file_path} ({file_type})")
                uploaded_count += 1
            asyncio.sleep(0.1)  # Small delay to avoid rate limiting
        ...
        while retry_count < max_retries:
            asyncio.sleep(3)
            list_result = agentbay.context.list_files(context_id, "/")
        ...
        for file in files_to_delete:
            delete_result = agentbay.context.delete_file(context_id, file.file_path)
        ...
        final_list_result = agentbay.context.list_files(context_id, "/")
        ...
        if context:
            ...
            try:
                delete_result = agentbay.context.delete(context)
                if delete_result.success:
