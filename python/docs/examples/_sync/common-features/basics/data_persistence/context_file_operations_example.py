...
def example_1_context_setup(agent_bay: AgentBay) -> Tuple:
...
def example_2_file_upload(agent_bay: AgentBay, context, test_files: List[str]):
...
def example_3_file_listing(agent_bay: AgentBay, context):
...
def example_4_file_download(agent_bay: AgentBay, context, file_names: List[str], download_dir: str):
...
def example_5_content_verification(temp_dir: str, download_dir: str, file_names: List[str]):
...
def example_6_selective_deletion(agent_bay: AgentBay, context):
    ...
    try:
        delete_result = agent_bay.context.delete_file(context.id, "/config.yaml")
    ...
    try:
        delete_result = agent_bay.context.delete_file(context.id, "/nonexistent.txt")
...
def example_7_cleanup(agent_bay: AgentBay, context, temp_dir: str, download_dir: str):
    ...
    try:
        ...
        if files_result.success:
            ...
            for file_entry in files_result.entries:
                ...
                try:
                    delete_result = agent_bay.context.delete_file(context.id, file_entry.file_path)
                    if delete_result.success:
...
def example_8_error_handling(agent_bay: AgentBay):
    """Example 8: Error handling and edge cases"""
    print("\n" + "="*70)
    ...
    try:
        result = agent_bay.context.get_file_upload_url("invalid-context-id", "/test.txt")
    ...
    try:
        ...
        context_result = agent_bay.context.create(f"error-test-{int(time.time())}")
        if context_result.success:
            context = context_result.context
            # Try to download non-existent file
            result = agent_bay.context.get_file_download_url(context.id, "/nonexistent.txt")
            if not result.success:
...
def main():
    ...
    try:
        ...
        context, temp_dir, test_files = example_1_context_setup(agent_bay)
        ...
        if context:
            ...
            uploaded_files = example_2_file_upload(agent_bay, context, test_files)

            # Example 3: File listing
            listed_files = example_3_file_listing(agent_bay, context)

            # Example 4: File download
            example_4_file_download(agent_bay, context, uploaded_files, download_dir)

            # Example 5: Content verification
            example_5_content_verification(temp_dir, download_dir, uploaded_files)

            # Example 6: Selective deletion
            example_6_selective_deletion(agent_bay, context)

            # Example 7: Cleanup
            example_7_cleanup(agent_bay, context, temp_dir, download_dir)

        # Example 8: Error handling
        example_8_error_handling(agent_bay)
