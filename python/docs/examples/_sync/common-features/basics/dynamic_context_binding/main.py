...
def main():
    ...
    try:
        dynamic_binding_demo(agent_bay)
...
def dynamic_binding_demo(agent_bay):
    ...
    try:
        ...
        read_result = session.file_system.read_file("/tmp/ctx-dynamic/test.txt")
        ...
        ctx_res_a = agent_bay.context.get(f"{context_name}-a", create=True)
        ctx_res_b = agent_bay.context.get(f"{context_name}-b", create=True)
        if ctx_res_a.success and ctx_res_b.success:
            ...
            bind_result2 = session.context.bind(cs_a, cs_b)
