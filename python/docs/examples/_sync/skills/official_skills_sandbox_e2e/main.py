...
def main() -> None:
    ...
    try:
        list_res = session.file_system.list_directory(SKILLS_ROOT)
        ...
        md_res = session.file_system.read_file(skill_md_path, format="text")
        ...
        w = session.file_system.write_file(out_path, report, mode="overwrite")
        if not w.success:
            raise RuntimeError(w.error_message or f"Failed to write {out_path}")

        v = session.file_system.read_file(out_path, format="text")
