...
@pytest.mark.asyncio
async def test_context_create_and_delete(agent_bay):
    """Test creating and deleting a context."""
    context_name = f"test-context-{uuid4().hex[:8]}"
    
    # Create context using create=True parameter
    create_result = await agent_bay.context.get(name=context_name, create=True)
...
@pytest.mark.asyncio
async def test_context_get_existing(agent_bay):
    ...
    try:
        ...
        get_result = await agent_bay.context.get(name=context_name)
...
@pytest.mark.asyncio
async def test_context_get_nonexistent(agent_bay):
    ...
    get_result = await agent_bay.context.get(name=context_name, create=False)
...
@pytest.mark.asyncio
async def test_context_list(agent_bay):
...
@pytest.mark.asyncio
async def test_context_update(agent_bay):
    ...
    try:
        ...
        update_result = await agent_bay.context.update(context)
...
@pytest.mark.asyncio
async def test_context_with_session(agent_bay):
...
@pytest.mark.asyncio
async def test_context_file_operations(agent_bay):
    ...
    try:
        # Get upload URL
        upload_url_result = await agent_bay.context.get_file_upload_url(
            context.id,
            "/test_file.txt"
        )
        ...
        try:
            download_url_result = await agent_bay.context.get_file_download_url(
                context.id,
                "/test_file.txt"
            )
