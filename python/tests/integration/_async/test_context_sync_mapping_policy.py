...
async def test_context_sync_with_mapping_policy(api_key):
    ...
    context_result = await ab.context.get(name=context_name, create=True)
    ...
    try:
        ...
        windows_dir_result = await windows_session.file_system.create_directory(
            windows_path
        )
        assert windows_dir_result.request_id is not None
        ...
        windows_cmd_result = await windows_session.command.execute_command(
            create_file_cmd
        )
        assert windows_cmd_result is not None
        ...
        verify_result = await windows_session.command.execute_command(verify_file_cmd)
        assert verify_result is not None
        print(f"Windows file content: {verify_result.output}")
        assert test_content in verify_result.output, \
        ...
        windows_delete_result = await ab.delete(windows_session)
        print(
        ...
        linux_session_result = await ab.create(linux_session_params)
        assert linux_session_result.session is not None
        ...
        try:
            ...
            check_result = await linux_session.command.execute_command(check_file_cmd)
            assert check_result is not None
            print(f"Linux file check result: {check_result.output}")

            # Verify file exists
            assert "FILE_EXISTS" in check_result.output, \
            ...
            read_result = await linux_session.command.execute_command(read_file_cmd)
            ...
            # Verify context info
            context_info = await linux_session.context.info()
            ...
            delete_result = await ab.delete(linux_session)
            print(
        ...
        delete_context_result = await ab.context.delete(context)
        print(
