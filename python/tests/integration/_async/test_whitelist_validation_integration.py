...
class TestWhiteListValidationIntegration:
    ...
    async def agent_bay(self):
        api_key = get_test_api_key()
        return AsyncAgentBay(api_key=api_key)

    @pytest_asyncio.fixture
    async def test_context(self, agent_bay):
        context_result = await agent_bay.context.get("test-wildcard-validation", create=True)
    ...
    async def test_create_session_with_wildcard_in_path_should_fail(
        ...
        with pytest.raises(ValueError) as exc_info:
            ...
            await agent_bay.create(CreateSessionParams(context_syncs=[context_sync]))
    ...
    async def test_create_session_with_valid_paths_should_succeed(
        ...
        await agent_bay.delete(session_result.session)

    @pytest.mark.asyncio
    async def test_validation_happens_before_api_call(self, agent_bay, test_context):
        with pytest.raises(ValueError) as exc_info:
