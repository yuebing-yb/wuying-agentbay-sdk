...
def validate_cursor_position(
    cursor_result, screen_result, expected_x: int, expected_y: int, tolerance: int
) -> bool:
    """
    ...
    if (
        cursor_data.get("x", -1) < 0
        or cursor_data.get("y", -1) < 0
        or cursor_data.get("x", 0) >= screen_data.get("width", 0)
        or cursor_data.get("y", 0) >= screen_data.get("height", 0)
    ):
        return False
...
def validate_screen_size(screen_result) -> bool:
    ...
    return (
        ...
        and height > 0
        and width <= 10000
        and height <= 10000  # Max reasonable size
        and dpi > 0
        and dpi <= 10
...
def validate_ui_elements_changed(
    before: List[Dict[str, Any]], after: List[Dict[str, Any]], tolerance: float
) -> bool:
    """
...
def validate_app_launched(
    before_ui: List[Dict[str, Any]], after_ui: List[Dict[str, Any]]
) -> bool:
    """
...
def validate_element_bounds(
    elem: Dict[str, Any], screen_width: int, screen_height: int
) -> bool:
    """
...
def safe_screenshot(
    computer_or_mobile, test_name: str
...
def wait_with_timeout(
    condition_func, timeout: float, check_interval: float = 0.1
) -> bool:
    """
