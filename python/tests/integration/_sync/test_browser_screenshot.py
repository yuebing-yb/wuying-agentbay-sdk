...
class TestBrowserScreenshotIntegration(unittest.TestCase):
    ...
    def _create_page_and_navigate(self, url: str) -> Any:
        """Helper method to create a page and navigate to a URL."""
        if not PLAYWRIGHT_AVAILABLE or sync_playwright is None:
            raise unittest.SkipTest("Playwright not available")
            
        self.playwright = sync_playwright().start()
        self.browser_instance = self.playwright.chromium.connect_over_cdp(self.endpoint_url)
    ...
    def _close_page_and_browser(self, page: Any):
    ...
    def test_screenshot_with_valid_page(self):
        ...
        page = self._create_page_and_navigate("https://www.aliyun.com")
        try:
            # Take screenshot with default full_page=False
            screenshot_data = self.browser.screenshot(page)
            ...
            self._close_page_and_browser(page)
    ...
    def test_screenshot_with_full_page(self):
        ...
        try:
            ...
            screenshot_data = self.browser.screenshot(page, full_page=True)
    ...
    def test_screenshot_with_custom_options(self):
        ...
        try:
            ...
            screenshot_data = self.browser.screenshot(
                page, 
    ...
    def test_screenshot_with_multiple_pages(self):
        ...
        for i, url in enumerate(urls):
            try:
                page = self._create_page_and_navigate(url)
                try:
                    # Take screenshot with default settings
                    # Use a shorter timeout for testing to avoid long waits
                    screenshot_data = self.browser.screenshot(page, timeout=10000)
