...
@pytest.mark.asyncio
def test_python_js_file_interop(session):
...
@pytest.mark.asyncio
def test_cross_language_data_exchange(session):
...
@pytest.mark.asyncio
def test_sequential_execution(session):
    ...
    # 1. Python
    py_result = session.code.run_code("print('Step 1: Python')", "python")
    ...
    # 3. Python again
    py_result2 = session.code.run_code("print('Step 3: Python again')", "python")
