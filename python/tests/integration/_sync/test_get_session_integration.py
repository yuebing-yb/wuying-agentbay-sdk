...
def test_get_session_api():
    ...
    try:
        ...
        assert (
            get_session_result.http_status_code == 200
        ), f"Expected HttpStatusCode 200, got {get_session_result.http_status_code}"
        assert (
            get_session_result.code == "ok"
        ), f"Expected Code 'ok', got '{get_session_result.code}'"
        ...
        assert (
            get_session_result.data.session_id == session_id
        ), f"Expected SessionID {session_id}, got {get_session_result.data.session_id}"
        ...
        assert (
            get_session_result.data.app_instance_id
        ), "AppInstanceID should not be empty"
