...
@pytest.mark.sync
def test_mobile_get_task_status_success(mobile_agent_session):
...
@pytest.mark.sync
def test_mobile_terminate_task_success(mobile_agent_session):
    ...
    try:
        ...
        retry_times = 0
        while retry_times < max_try_times:
            time.sleep(1)  # Wait a moment for termination to process
...
@pytest.mark.sync
def test_mobile_execute_task_and_wait_timeout_termination(mobile_agent_session):
    ...
    try:
        ...
        if new_task_result.task_id:
            try:
                time.sleep(2)
                status = agent.mobile.get_task_status(new_task_result.task_id)
                if status.success and status.task_status == "running":
                    terminate_result = agent.mobile.terminate_task(new_task_result.task_id)
                    if terminate_result.success:
