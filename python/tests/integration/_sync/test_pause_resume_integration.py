...
@pytest.mark.sync
def test_agentbay_beta_pause_and_beta_resume_async(agent_bay: AgentBay, session):
    pause_result = agent_bay.beta_pause(session, timeout=600, poll_interval=2.0)
...
async def _create_test_session(agent_bay: AgentBay):
    session_name = f"test-beta-pause-resume-{uuid4().hex[:8]}"
    params = CreateSessionParams(
        image_id="linux_latest",
        labels={"project": "piaoyun-demo", "environment": "testing", "name": session_name},
...
async def _safe_cleanup_session(agent_bay: AgentBay, session) -> None:
    if session is None:
        return
...
@pytest.fixture
async def session(agent_bay: AgentBay):
...
@pytest.mark.sync
async def test_beta_pause_and_delete_session_success(agent_bay: AgentBay):
    s = await _create_test_session(agent_bay)
    try:
...
@pytest.mark.sync
async def test_agentbay_beta_pause_and_beta_resume_async(agent_bay: AgentBay, session):
...
@pytest.mark.sync
async def test_beta_pause_nonexistent_session(agent_bay: AgentBay):
    fake_session_id = f"session-nonexistent-{uuid4().hex[:8]}"
