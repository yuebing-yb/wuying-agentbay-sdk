...
class TestExtensionBrowserIntegration(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        # Create extension repository context
        cls.context_name = f"test-extensions-{int(time.time())}"
        context_result = cls.agent_bay.context.get(cls.context_name, create=True)
        if not context_result.success or not context_result.context:
            raise unittest.SkipTest("Failed to create extension repository context")
        
        cls.extension_context = context_result.context
        cls.context_id = cls.extension_context.id
        ...
        # Initialize ExtensionsService
        cls.extensions_service = ExtensionsService(cls.agent_bay, cls.context_id)
    ...
    def tearDownClass(cls):
        ...
        # Clean up contexts
        if hasattr(cls, "extension_context"):
            try:
                cls.agent_bay.context.delete(cls.extension_context)
                print(f"Extension context deleted: {cls.context_id}")
            except Exception as e:
...
"""
    ...
    # Helper Methods
    def _create_context_sync(self, extension_ids: List[str]) -> ContextSync:
        """Create ContextSync with extension whitelist policy."""
        print(f"ext_id : {[ext_id for ext_id in extension_ids]}")
        white_lists = [WhiteList(path=f"{ext_id}", exclude_paths=[]) for ext_id in extension_ids]
    ...
    def _create_browser_session(self, extension_ids: List[str]):
        """Create browser session with extension synchronization."""
        # Create context sync with extension IDs
        context_sync = self._create_context_sync(extension_ids)
        
        # Create session with extension sync and browser context
        session_params = CreateSessionParams(
            labels={"test_type": "extension_integration"},
            image_id="imgc-wucyOiPmeV2Z753lq",
            context_syncs=[context_sync],
            browser_context=BrowserContext(self.browser_context_id, auto_upload=True),
            is_vpc=False
        )
