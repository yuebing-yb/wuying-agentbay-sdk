...
from agentbay.exceptions import AuthenticationError, APIError
    ...
    def test_create_list_delete(self):
        ...
        for s in sessions:
            if s.session_id == session.session_id:
                found = True
                break
        self.assertTrue(found, f"Created session with ID {session.session_id} not found in sessions list")
        ...
        for s in sessions:
            self.assertNotEqual(s.session_id, session.session_id, 
    ...
    def test_command(self):
        ...
        if self.session.command:
            ...
            try:
                ...
                self.assertNotIn("tool not found", response.lower(), "Command.ExecuteCommand returned 'tool not found'")
            except Exception as e:
    ...
    def test_filesystem(self):
        ...
        if self.session.file_system:
            ...
            try:
                ...
                # Check if response contains "tool not found"
                self.assertNotIn("tool not found", content.lower(), "FileSystem.ReadFile returned 'tool not found'")
    ...
    def test_adb_shell(self):
        ...
        if self.session.adb:
            ...
            try:
                ...
                self.assertIsNotNone(response)
                # Check if response contains "tool not found"
                self.assertNotIn("tool not found", response.lower(), "Adb.Shell returned 'tool not found'")
            except Exception as e:
            ...
            try:
                ...
                print(f"ADB getprop execution result length: {len(prop_response)} bytes")
                self.assertIsNotNone(prop_response)
                # Check if response contains "tool not found"
                self.assertNotIn("tool not found", prop_response.lower(), "Adb.Shell returned 'tool not found'")
            except Exception as e:
