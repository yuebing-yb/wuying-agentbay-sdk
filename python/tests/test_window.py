            ...
            try:
                ...
                if len(root_windows) == 0:
                    ...
                    for window in root_windows:
                        self.assertGreater(window.window_id, 0, f"Found window with invalid ID: {window.window_id}")
                
                # Check if response contains "tool not found"
                if len(root_windows) > 0:
                    self.assertFalse(contains_tool_not_found(root_windows[0].title), 
    ...
    def test_get_active_window(self):
        ...
        if hasattr(self.session, 'window') and self.session.window:
            ...
            try:
                ...
                # Verify window properties
                self.assertGreater(active_window.window_id, 0, f"Active window has invalid ID: {active_window.window_id}")
                self.assertGreater(active_window.pid, 0, f"Active window has invalid PID: {active_window.pid}")
                
                # Check if response contains "tool not found"
                self.assertFalse(contains_tool_not_found(active_window.title) or contains_tool_not_found(active_window.pname), 
