...
class TestAsyncAgentComputer(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_computer_task_execute_and_wait_success(self):
        ...
        result = await self.agent.computer.execute_task_and_wait("Hello, Computer Agent", self.timeout)
...
class TestAsyncAgentBrowser(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_browser_task_execute_and_wait_success(self):
        ...
        result = await self.agent.browser.execute_task_and_wait("Hello, Browser Agent", self.timeout)
        self.assertIsInstance(result, ExecutionResult)
        self.assertTrue(result.success)
        self.assertEqual(result.request_id, "request-123")
        self.assertEqual(result.task_status, "finished")
...
class TestAsyncAgentMobile(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        self.session = DummySession()
        self.agent = AsyncAgent(self.session)
        timeout = os.environ.get("AGENT_TASK_TIMEOUT")
    ...
    @pytest.mark.asyncio
    async def test_mobile_task_execute_success(self):
        """
        ...
        result = await self.agent.mobile.execute_task(
            "Open WeChat app", max_steps=100
        )
        from agentbay._async.agent import TaskExecution
    ...
    @pytest.mark.asyncio
    async def test_mobile_task_execute_and_wait_success(self):
        """
    ...
    @pytest.mark.asyncio
    async def test_mobile_execute_task_and_wait_error(self):
        """
    ...
    @pytest.mark.asyncio
    async def test_mobile_task_execute_timeout(self):
        """
    ...
    async def test_mobile_task_terminate_success(self):
        ...
        result = await self.agent.mobile.terminate_task("task-123")
        self.assertIsInstance(result, ExecutionResult)
        self.assertTrue(result.success)
        self.assertEqual(result.request_id, "request-123")
    ...
    @pytest.mark.asyncio
    async def test_mobile_task_terminate_error(self):
        """
    ...
    @pytest.mark.asyncio
    async def test_mobile_execute_task_with_default_params(self):
        """
        ...
        result = await self.agent.mobile.execute_task("Open WeChat app")
        from agentbay._async.agent import TaskExecution
        self.assertIsInstance(result, TaskExecution)
        self.assertEqual(result.task_id, "task-123")
