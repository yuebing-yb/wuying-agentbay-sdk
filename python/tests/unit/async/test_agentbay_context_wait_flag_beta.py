...
class TestAsyncAgentBayContextWaitFlagBeta(unittest.IsolatedAsyncioTestCase):
    @pytest.mark.asyncio
    async def test_wait_for_context_synchronization_respects_wait_context_ids(self):
        agent = AsyncAgentBay(api_key="dummy")
        ...
        with patch("agentbay._async.agentbay.asyncio.sleep", new=_no_sleep):
            await agent._wait_for_context_synchronization(
                session,
