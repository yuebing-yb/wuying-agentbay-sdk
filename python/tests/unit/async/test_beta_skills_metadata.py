...
class TestBetaSkillsMetadata(unittest.IsolatedAsyncioTestCase):
    ...
    @pytest.mark.asyncio
    async def test_list_metadata_parses_response(self):
        with patch("agentbay._async.agentbay._load_config") as mock_load_config, patch(
            "agentbay._async.agentbay.mcp_client"
        ) as mock_mcp_client:
            mock_load_config.return_value = {
            ...
            async def _fake_list_skill_meta_data(req):
            ...
            agent_bay.client.list_skill_meta_data_async = AsyncMock(side_effect=_fake_list_skill_meta_data)
