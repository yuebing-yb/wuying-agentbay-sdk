...
class TestAsyncCommand(unittest.IsolatedAsyncioTestCase):
    ...
    @pytest.mark.asyncio
    async def test_run_alias_calls_execute_command(self):
        from agentbay import McpToolResult
        ...
        result = await self.command.run("echo test", timeout_ms=1234, cwd="/tmp")
        self.assertIsInstance(result, CommandResult)
        self.session.call_mcp_tool.assert_called_once()
    ...
    @pytest.mark.asyncio
    async def test_exec_alias_calls_execute_command(self):
        from agentbay import McpToolResult
        ...
        result = await self.command.exec("echo test", envs={"A": "B"})
        self.assertIsInstance(result, CommandResult)
        self.session.call_mcp_tool.assert_called_once()
    ...
    async def test_execute_command_error_with_json_in_error_message(self):
        """
        ...
        result = await self.command.execute_command("cat /nonexistent_file_12345")
        self.assertIsInstance(result, CommandResult)
        self.assertFalse(result.success)
        self.assertEqual(result.request_id, "request-123")
    ...
    async def test_execute_command_custom_timeout(self):
        """
        ...
        result = await self.command.execute_command("ls -la", timeout_ms=120000)
        self.assertIsInstance(result, CommandResult)
        self.assertTrue(result.success)
