...
class TestAsyncFileSystem(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_delete_file_success(self):
        ...
        result = await self.fs.delete_file("/path/to/file.txt")
        self.assertIsInstance(result, BoolResult)
        self.assertTrue(result.success)
        self.assertEqual(result.request_id, "request-123")
        self.assertTrue(result.data)
    ...
    async def test_edit_file_error(self):
        """
    ...
    async def test_read_multiple_files_success(self):
        """
    ...
    async def test_search_files_with_exclude(self):
        """
