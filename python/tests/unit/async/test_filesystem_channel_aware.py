...
class TestIsUsingLinkUrl(unittest.IsolatedAsyncioTestCase):
    ...
    def test_returns_false_when_session_lacks_methods(self):
        ...
        session.call_mcp_tool = AsyncMock()
        fs = AsyncFileSystem(session)
        self.assertFalse(fs._is_using_link_url())


class TestReadFileHttpChannel(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_read_file_http_skips_get_file_info(self, mock_get_file_info, mock_read_chunk):
        ...
        result = await self.fs.read_file("/tmp/test.txt")
    ...
    @patch("agentbay._async.filesystem.AsyncFileSystem._read_file_chunk")
    async def test_read_file_http_binary_format(self, mock_read_chunk):
        ...
        result = await self.fs.read_file("/tmp/binary.bin", format="bytes")
    ...
    @patch("agentbay._async.filesystem.AsyncFileSystem._read_file_chunk")
    async def test_read_file_http_error_handling(self, mock_read_chunk):
        """On HTTP channel, read_file should handle errors gracefully."""
        mock_read_chunk.return_value = FileContentResult(
            request_id="req-1", success=False, error_message="File not found"
        )

        result = await self.fs.read_file("/tmp/nonexistent.txt")

        self.assertFalse(result.success)
        self.assertIn("File not found", result.error_message)

    @patch("agentbay._async.filesystem.AsyncFileSystem._read_file_chunk")
    async def test_read_file_http_exception_handling(self, mock_read_chunk):
...
class TestWriteFileHttpChannel(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_write_file_http_single_call(self, mock_write_chunk):
...
class TestReadFileMqttChannel(unittest.IsolatedAsyncioTestCase):
    ...
    @patch("agentbay._async.filesystem.AsyncFileSystem._read_file_chunk")
    @patch("agentbay._async.filesystem.AsyncFileSystem.get_file_info")
    async def test_read_file_mqtt_calls_get_file_info(self, mock_get_file_info, mock_read_chunk):
    ...
    @patch("agentbay._async.filesystem.AsyncFileSystem._read_file_chunk")
    @patch("agentbay._async.filesystem.AsyncFileSystem.get_file_info")
    async def test_read_file_mqtt_chunks_large_file(self, mock_get_file_info, mock_read_chunk):
...
class TestWriteFileMqttChannel(unittest.IsolatedAsyncioTestCase):
    ...
    @patch("agentbay._async.filesystem.AsyncFileSystem._write_file_chunk")
    async def test_write_file_mqtt_chunks_large_content(self, mock_write_chunk):
    ...
    @patch("agentbay._async.filesystem.AsyncFileSystem._write_file_chunk")
    async def test_write_file_mqtt_small_content_single_call(self, mock_write_chunk):
        """On MQTT channel, write_file with small content should use single call."""
        mock_write_chunk.return_value = BoolResult(
            request_id="req-1", success=True, data=True
        )

        result = await self.fs.write_file("/tmp/small.txt", "small content")
