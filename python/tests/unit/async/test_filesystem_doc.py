...
async def main():
    ...
    try:
        ...
        print(
            f"Success: {create_result.success}, Data: {create_result.data}, RequestID: {create_result.request_id}"
        )

        print("\n=== Test 2: Write File ===")
        test_file = "/tmp/agentbay_test/test.txt"
        write_result = await filesystem.write_file(
            test_file, "Hello, AgentBay!", mode="overwrite"
        )
        print(
            f"Success: {write_result.success}, Data: {write_result.data}, RequestID: {write_result.request_id}"
        )
        ...
        print(
            f"Success: {edit_result.success}, Data: {edit_result.data}, RequestID: {edit_result.request_id}"
        )
        ...
        print(
            f"Entries (Total: {len(list_result.entries) if list_result.entries else 0}):"
        )
        ...
        print(
            f"Success: {move_result.success}, Data: {move_result.data}, RequestID: {move_result.request_id}"
        )

        print("\n=== Test 8: Read Multiple Files ===")
        await filesystem.write_file(
            "/tmp/agentbay_test/file1.txt", "Content 1", mode="overwrite"
        )
        await filesystem.write_file(
            "/tmp/agentbay_test/file2.txt", "Content 2", mode="overwrite"
        )
        ...
        print(
            f"Files read: {len(multi_read_result.contents) if multi_read_result.contents else 0}"
        )
        ...
        print(
            f"Found {len(search_result.matches) if search_result.matches else 0} files:"
        )
        ...
        await filesystem.create_directory(watch_dir)
        ...
        monitor_thread = filesystem.watch_directory(
            path=watch_dir, callback=on_change, interval=1.0
        )
        ...
        await filesystem.write_file(
            test_watch_file, "Modified watch test content", mode="overwrite"
        )
