...
from agentbay import AsyncFileSystem, FileChangeEvent, FileChangeResult
...
def _make_ws_client() -> MagicMock:
    ...
    handle = MagicMock()
    handle.wait_end_with_timeout = AsyncMock(return_value={"phase": "end"})
    ws.call_stream = AsyncMock(return_value=handle)
    ws.send_message = AsyncMock()
    return ws


async def _start_and_stop_thread(thread, *, start_sleep=1.5, stop_timeout=5.0):
    """Start a watch thread, wait, then stop it without blocking the event loop."""
    thread.start()
    await asyncio.sleep(start_sleep)
    thread.stop_event.set()
    await asyncio.to_thread(thread.join, stop_timeout)


class TestWatchDirectoryWsPushActivation(unittest.IsolatedAsyncioTestCase):
    """Test that WS push mode is selected when all prerequisites are met."""

    @pytest.mark.asyncio
    async def test_ws_push_selected_when_prerequisites_met(self):
        """When ws_url, token, server_name and running loop exist,
        the thread name should contain 'WS'."""
        session = _make_session()
        fs = AsyncFileSystem(session)
        mock_ws = _make_ws_client()
        session._get_ws_client = AsyncMock(return_value=mock_ws)

        with patch.object(fs, "_get_file_change", new_callable=AsyncMock) as mock_gfc:
            ...
            await _start_and_stop_thread(thread)
            self.assertFalse(thread.is_alive())

    @pytest.mark.asyncio
    async def test_polling_selected_when_no_ws_url(self):
        ...
        with patch.object(fs, "_get_file_change", new_callable=AsyncMock) as mock_gfc:
            ...
            await _start_and_stop_thread(thread, start_sleep=0.3)

    @pytest.mark.asyncio
    async def test_polling_selected_when_no_token(self):
    ...
    @pytest.mark.asyncio
    async def test_polling_selected_when_no_server_name(self):
...
class TestWatchDirectoryWsPushSubscription(unittest.IsolatedAsyncioTestCase):
    """Test the WS subscribe flow (call_stream + wait_end)."""

    @pytest.mark.asyncio
    async def test_subscribe_called_with_correct_params(self):
    ...
    @pytest.mark.asyncio
    async def test_register_callback_called_with_server_name(self):
    ...
    @pytest.mark.asyncio
    async def test_on_connection_state_change_registered(self):
...
class TestWatchDirectoryWsPushEventDispatch(unittest.IsolatedAsyncioTestCase):
    """Test that push events are correctly dispatched to user callback."""

    @pytest.mark.asyncio
    async def test_push_event_dispatched_to_callback(self):
    ...
    @pytest.mark.asyncio
    async def test_push_event_wrong_path_ignored(self):
    ...
    @pytest.mark.asyncio
    async def test_push_event_non_file_change_ignored(self):
    ...
    @pytest.mark.asyncio
    async def test_push_event_callback_exception_handled(self):
...
class TestWatchDirectoryWsPushUnsubscribe(unittest.IsolatedAsyncioTestCase):
    """Test cleanup on stop: unsubscribe_file_change and callback unregister."""

    @pytest.mark.asyncio
    async def test_unsubscribe_sent_on_stop(self):
    ...
    @pytest.mark.asyncio
    async def test_push_callback_unregistered_on_stop(self):
...
class TestWatchDirectoryWsPushFallback(unittest.IsolatedAsyncioTestCase):
    """Test fallback to polling when WS subscribe fails."""

    @pytest.mark.asyncio
    async def test_falls_back_to_polling_on_subscribe_error(self):
        """If WS subscribe raises, watch should fall back to polling
        and continue detecting changes via _get_file_change."""
        session = _make_session()
        fs = AsyncFileSystem(session)
        mock_ws = _make_ws_client()
        mock_ws.call_stream = AsyncMock(side_effect=Exception("WS subscribe failed"))
        session._get_ws_client = AsyncMock(return_value=mock_ws)
...
class TestWatchDirectoryWsStateListenerCleanup(unittest.IsolatedAsyncioTestCase):
    ...
    @pytest.mark.asyncio
    async def test_state_listener_cleanup_on_stop(self):
