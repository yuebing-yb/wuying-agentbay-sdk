...
from agentbay._async.git.git import AsyncGit
...
class DummySession:
    ...
    def __init__(self):
        ...
        self.command.execute_command = AsyncMock()
...
class TestAsyncGitHelpers(unittest.TestCase):
    ...
    def test_shell_escape_plain(self):
        assert AsyncGit._shell_escape("hello") == "'hello'"

    def test_shell_escape_single_quote(self):
        assert AsyncGit._shell_escape("it's") == "'it'\\''s'"

    def test_shell_escape_spaces(self):
        assert AsyncGit._shell_escape("hello world") == "'hello world'"

    def test_shell_escape_empty(self):
        assert AsyncGit._shell_escape("") == "''"

    # -- _build_git_command ------------------------------------------------

    def test_build_git_command_no_repo_path(self):
        session = DummySession()
        git = AsyncGit(session)
    ...
    def test_derive_repo_plain_name(self):
        assert AsyncGit._derive_repo_dir_from_url("repo") == "repo"

    def test_derive_repo_url_encoded(self):
        assert AsyncGit._derive_repo_dir_from_url(
    ...
    def test_derive_status_conflict(self):
        assert AsyncGit._derive_status("U", " ") == "conflict"

    def test_derive_status_renamed(self):
        assert AsyncGit._derive_status("R", " ") == "renamed"
    ...
    def test_derive_status_typechange(self):
        assert AsyncGit._derive_status("T", " ") == "typechange"
...
class TestAsyncGitParsers(unittest.TestCase):
    ...
    def test_parse_status_detached_head(self):
        result = AsyncGit._parse_git_status("## HEAD (detached at abc1234)")
        assert result.detached
        assert result.current_branch is None

    def test_parse_status_no_commits_yet(self):
        result = AsyncGit._parse_git_status("## No commits yet on main")
        assert result.current_branch == "main"
    ...
    def test_parse_log_empty(self):
        result = AsyncGit._parse_git_log("")
    ...
    def test_parse_log_single(self):
        ...
        result = AsyncGit._parse_git_log(record)
    ...
    def test_parse_log_multiple(self):
        ...
        result = AsyncGit._parse_git_log(records)
    ...
    def test_parse_branches_empty(self):
        result = AsyncGit._parse_git_branches("")
        assert len(result.branches) == 0
        assert result.current == ""

    def test_parse_branches_single_current(self):
        output = "main\t*"
        result = AsyncGit._parse_git_branches(output)
...
class TestAsyncGitClassifyError(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.git = AsyncGit(self.session)
...
class TestAsyncGitAvailability(unittest.IsolatedAsyncioTestCase):
    """Tests for _ensure_git_available."""

    @pytest.mark.asyncio
    async def test_git_available_success(self):
        session = DummySession()
        git = AsyncGit(session)
        session.command.execute_command.return_value = _ok(stdout="git version 2.39.0")
        await git._ensure_git_available()
        assert git._git_available is True

    @pytest.mark.asyncio
    async def test_git_available_cached(self):
        session = DummySession()
        git = AsyncGit(session)
    ...
    @pytest.mark.asyncio
    async def test_git_not_available(self):
        session = DummySession()
        git = AsyncGit(session)
...
class TestAsyncGitClone(unittest.IsolatedAsyncioTestCase):
    """Tests for clone()."""
    ...
    async def test_clone_basic(self):
        ...
        result = await self.git.clone("https://github.com/user/repo.git")
    ...
    @pytest.mark.asyncio
    async def test_clone_with_path(self):
        self.session.command.execute_command.return_value = _ok()
    ...
    @pytest.mark.asyncio
    async def test_clone_with_branch_and_depth(self):
        self.session.command.execute_command.return_value = _ok()
        await self.git.clone("https://github.com/user/repo.git", branch="main", depth=1)
        cmd = self.session.command.execute_command.call_args[0][0]
    ...
    @pytest.mark.asyncio
    async def test_clone_default_timeout(self):
        self.session.command.execute_command.return_value = _ok()
        await self.git.clone("https://github.com/user/repo.git")
        kwargs = self.session.command.execute_command.call_args[1]
        assert kwargs["timeout_ms"] == 300000

    @pytest.mark.asyncio
    async def test_clone_custom_timeout(self):
        self.session.command.execute_command.return_value = _ok()
        await self.git.clone("https://github.com/user/repo.git", timeout_ms=600000)
        kwargs = self.session.command.execute_command.call_args[1]
        assert kwargs["timeout_ms"] == 600000

    @pytest.mark.asyncio
    async def test_clone_env(self):
        self.session.command.execute_command.return_value = _ok()
        await self.git.clone("https://github.com/user/repo.git")
        kwargs = self.session.command.execute_command.call_args[1]
    ...
    @pytest.mark.asyncio
    async def test_clone_auth_error(self):
        self.session.command.execute_command.return_value = _fail(
            stderr="fatal: Authentication failed"
        )
        with pytest.raises(GitAuthError):
            await self.git.clone("https://github.com/user/private.git")
    @pytest.mark.asyncio
    async def test_clone_depth_zero(self):
        """depth=0 is falsy, should NOT add --depth flag."""
        self.session.command.execute_command.return_value = _ok()
        await self.git.clone("https://github.com/user/repo.git", depth=0)
        cmd = self.session.command.execute_command.call_args[0][0]
        assert "'--depth'" not in cmd

    @pytest.mark.asyncio
    async def test_clone_depth_negative(self):
        """Negative depth is truthy, will be passed to git."""
        self.session.command.execute_command.return_value = _ok()
        await self.git.clone("https://github.com/user/repo.git", depth=-1)
        cmd = self.session.command.execute_command.call_args[0][0]
        assert "'--depth'" in cmd
        assert "'-1'" in cmd

    @pytest.mark.asyncio
    async def test_clone_generic_error(self):
        self.session.command.execute_command.return_value = _fail(
...
class TestAsyncGitInit(unittest.IsolatedAsyncioTestCase):
    """Tests for init()."""
    ...
    async def test_init_basic(self):
        self.session.command.execute_command.return_value = _ok()
        result = await self.git.init("/tmp/repo")
    ...
    @pytest.mark.asyncio
    async def test_init_with_initial_branch(self):
        self.session.command.execute_command.return_value = _ok()
        await self.git.init("/tmp/repo", initial_branch="main")
        cmd = self.session.command.execute_command.call_args[0][0]
        assert "'--initial-branch'" in cmd
        assert "'main'" in cmd

    @pytest.mark.asyncio
    async def test_init_bare(self):
        self.session.command.execute_command.return_value = _ok()
        await self.git.init("/tmp/repo", bare=True)
        cmd = self.session.command.execute_command.call_args[0][0]
        assert "'--bare'" in cmd
    ...
    async def test_init_failure(self):
        ...
        with pytest.raises(GitError):
            await self.git.init("/tmp/repo")


class TestAsyncGitAdd(unittest.IsolatedAsyncioTestCase):
    """Tests for add()."""
    ...
    @pytest.mark.asyncio
    async def test_add_stage_all_default(self):
        self.session.command.execute_command.return_value = _ok()
        await self.git.add("/repo")
        cmd = self.session.command.execute_command.call_args[0][0]
        assert "'-A'" in cmd

    @pytest.mark.asyncio
    async def test_add_specific_files(self):
        self.session.command.execute_command.return_value = _ok()
        await self.git.add("/repo", files=["a.txt", "b.txt"])
        cmd = self.session.command.execute_command.call_args[0][0]
        assert "'--'" in cmd
        assert "'a.txt'" in cmd
        assert "'b.txt'" in cmd
    ...
    @pytest.mark.asyncio
    async def test_add_stage_all_false(self):
        self.session.command.execute_command.return_value = _ok()
        await self.git.add("/repo", stage_all=False)
        cmd = self.session.command.execute_command.call_args[0][0]
        assert "'.'" in cmd
        assert "'-A'" not in cmd

    @pytest.mark.asyncio
    async def test_add_not_a_repo(self):
        self.session.command.execute_command.return_value = _fail(
            stderr="fatal: not a git repository", exit_code=128
        )
        with pytest.raises(GitNotARepoError):
            await self.git.add("/not-a-repo")


class TestAsyncGitCommit(unittest.IsolatedAsyncioTestCase):
    """Tests for commit()."""
    ...
    @pytest.mark.asyncio
    async def test_commit_basic(self):
        self.session.command.execute_command.return_value = _ok(
            stdout="[main abc1234] initial commit"
        )
        result = await self.git.commit("/repo", "initial commit")
        assert isinstance(result, GitCommitResult)
        assert result.commit_hash == "abc1234"

    @pytest.mark.asyncio
    async def test_commit_with_author(self):
        self.session.command.execute_command.return_value = _ok(
    ...
    @pytest.mark.asyncio
    async def test_commit_allow_empty(self):
        self.session.command.execute_command.return_value = _ok(
            stdout="[main 111aaaa] empty"
        )
        await self.git.commit("/repo", "empty", allow_empty=True)
        cmd = self.session.command.execute_command.call_args[0][0]
        assert "'--allow-empty'" in cmd
    @pytest.mark.asyncio
    async def test_commit_message_with_single_quotes(self):
        """Verify single quotes in commit message are properly escaped."""
        self.session.command.execute_command.return_value = _ok(
            stdout="[main aaa1111] fix: handle 'quoted' case"
        )
        await self.git.commit("/repo", "fix: handle 'quoted' case")
        cmd = self.session.command.execute_command.call_args[0][0]
    ...
    async def test_commit_failure(self):
        ...
        with pytest.raises(GitError):
            await self.git.commit("/repo", "msg")

class TestAsyncGitStatus(unittest.IsolatedAsyncioTestCase):
    """Tests for status()."""
    ...
    async def test_status_clean(self):
        ...
        result = await self.git.status("/repo")
        assert isinstance(result, GitStatusResult)
        assert result.current_branch == "main"
        assert result.is_clean

    @pytest.mark.asyncio
    async def test_status_with_changes(self):
        self.session.command.execute_command.return_value = _ok(
            stdout="## main\n?? new.txt\nA  added.txt"
        )
        result = await self.git.status("/repo")
    ...
    @pytest.mark.asyncio
    async def test_status_not_a_repo(self):
        self.session.command.execute_command.return_value = _fail(
            stderr="fatal: not a git repository", exit_code=128
        )
        with pytest.raises(GitNotARepoError):
            await self.git.status("/not-a-repo")


class TestAsyncGitLog(unittest.IsolatedAsyncioTestCase):
    """Tests for log()."""
    ...
    async def test_log_default(self):
        ...
        result = await self.git.log("/repo")
        assert isinstance(result, GitLogResult)
        assert len(result.entries) == 1

    @pytest.mark.asyncio
    async def test_log_max_count(self):
        self.session.command.execute_command.return_value = _ok(stdout="")
        await self.git.log("/repo", max_count=5)
        cmd = self.session.command.execute_command.call_args[0][0]
    ...
    async def test_log_failure(self):
        ...
        with pytest.raises(GitError):
            await self.git.log("/repo")


class TestAsyncGitBranches(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_list_branches(self):
        ...
        result = await self.git.list_branches("/repo")
    ...
    async def test_list_branches_failure(self):
        ...
        with pytest.raises(GitError):
            await self.git.list_branches("/repo")

    @pytest.mark.asyncio
    async def test_create_branch_checkout(self):
        self.session.command.execute_command.return_value = _ok()
        await self.git.create_branch("/repo", "feature/x")
        cmd = self.session.command.execute_command.call_args[0][0]
    ...
    @pytest.mark.asyncio
    async def test_create_branch_no_checkout(self):
        self.session.command.execute_command.return_value = _ok()
        await self.git.create_branch("/repo", "feature/x", checkout=False)
        cmd = self.session.command.execute_command.call_args[0][0]
        assert "'branch'" in cmd
        assert "'feature/x'" in cmd
        assert "'-b'" not in cmd
    ...
    async def test_create_branch_failure(self):
        ...
        with pytest.raises(GitError):
            await self.git.create_branch("/repo", "x")

    @pytest.mark.asyncio
    async def test_checkout_branch(self):
        self.session.command.execute_command.return_value = _ok()
        await self.git.checkout_branch("/repo", "main")
        cmd = self.session.command.execute_command.call_args[0][0]
        assert "'checkout'" in cmd
        assert "'main'" in cmd

    @pytest.mark.asyncio
    async def test_checkout_branch_failure(self):
        self.session.command.execute_command.return_value = _fail(
            stderr="error: pathspec 'nonexistent' did not match"
        )
        with pytest.raises(GitError):
            await self.git.checkout_branch("/repo", "nonexistent")

    @pytest.mark.asyncio
    async def test_delete_branch_default(self):
        self.session.command.execute_command.return_value = _ok()
        await self.git.delete_branch("/repo", "old-branch")
        cmd = self.session.command.execute_command.call_args[0][0]
        assert "'-d'" in cmd
        assert "'old-branch'" in cmd

    @pytest.mark.asyncio
    async def test_delete_branch_force(self):
        self.session.command.execute_command.return_value = _ok()
        await self.git.delete_branch("/repo", "old-branch", force=True)
        cmd = self.session.command.execute_command.call_args[0][0]
        assert "'-D'" in cmd
    ...
    async def test_delete_branch_failure(self):
        ...
        with pytest.raises(GitError):
            await self.git.delete_branch("/repo", "main")


class TestAsyncGitRemote(unittest.IsolatedAsyncioTestCase):
    ...
    @pytest.mark.asyncio
    async def test_remote_add_basic(self):
        self.session.command.execute_command.return_value = _ok()
        await self.git.remote_add("/repo", "origin", "https://github.com/user/repo.git")
        cmd = self.session.command.execute_command.call_args[0][0]
    ...
    @pytest.mark.asyncio
    async def test_remote_add_with_fetch(self):
        self.session.command.execute_command.return_value = _ok()
        await self.git.remote_add(
    ...
    @pytest.mark.asyncio
    async def test_remote_add_overwrite(self):
        self.session.command.execute_command.return_value = _ok()
        await self.git.remote_add(
            "/repo",
            "origin",
    ...
    @pytest.mark.asyncio
    async def test_remote_add_overwrite_with_fetch(self):
        self.session.command.execute_command.return_value = _ok()
        await self.git.remote_add(
            "/repo",
            "origin",
    ...
    @pytest.mark.asyncio
    async def test_remote_add_failure(self):
        self.session.command.execute_command.return_value = _fail(
            stderr="fatal: remote origin already exists"
        )
        with pytest.raises(GitError):
            await self.git.remote_add("/repo", "origin", "url")

    @pytest.mark.asyncio
    async def test_remote_get_success(self):
        self.session.command.execute_command.return_value = _ok(
            stdout="https://github.com/user/repo.git\n"
        )
        url = await self.git.remote_get("/repo", "origin")
        assert url == "https://github.com/user/repo.git"

    @pytest.mark.asyncio
    async def test_remote_get_not_exist(self):
        self.session.command.execute_command.return_value = _fail(
            stderr="fatal: No such remote 'nonexistent'"
        )
        result = await self.git.remote_get("/repo", "nonexistent")
        assert result is None

    @pytest.mark.asyncio
    async def test_remote_get_other_error(self):
        self.session.command.execute_command.return_value = _fail(
            stderr="fatal: not a git repository"
        )
        with pytest.raises(GitError):
            await self.git.remote_get("/repo", "origin")
    ...
    async def test_reset_invalid_mode(self):
        ...
        with pytest.raises(ValueError, match="Invalid reset mode"):
            await self.git.reset("/repo", mode="invalid")
        # Ensure no git command was executed
        self.session.command.execute_command.assert_not_called()

    @pytest.mark.asyncio
    async def test_reset_failure(self):
        self.session.command.execute_command.return_value = _fail(stderr="fatal: error")
        with pytest.raises(GitError):
            await self.git.reset("/repo")

    @pytest.mark.asyncio
    async def test_reset_modes(self):
        for mode in ("soft", "mixed", "hard", "merge", "keep"):
            self.session.command.execute_command.return_value = _ok()
            await self.git.reset("/repo", mode=mode)
            cmd = self.session.command.execute_command.call_args[0][0]
            assert f"'--{mode}'" in cmd

    @pytest.mark.asyncio
    async def test_reset_with_target(self):
        self.session.command.execute_command.return_value = _ok()
        await self.git.reset("/repo", mode="hard", target="HEAD~2")
        cmd = self.session.command.execute_command.call_args[0][0]
        assert "'--hard'" in cmd
        assert "'HEAD~2'" in cmd

class TestAsyncGitRestore(unittest.IsolatedAsyncioTestCase):
    """Tests for restore()."""
    ...
    @pytest.mark.asyncio
    async def test_restore_worktree_default(self):
        self.session.command.execute_command.return_value = _ok()
        await self.git.restore("/repo", ["file.txt"])
        cmd = self.session.command.execute_command.call_args[0][0]
    ...
    @pytest.mark.asyncio
    async def test_restore_staged(self):
        self.session.command.execute_command.return_value = _ok()
        await self.git.restore("/repo", ["file.txt"], staged=True)
        cmd = self.session.command.execute_command.call_args[0][0]
    ...
    @pytest.mark.asyncio
    async def test_restore_staged_and_worktree(self):
        self.session.command.execute_command.return_value = _ok()
        await self.git.restore("/repo", ["file.txt"], staged=True, worktree=True)
        cmd = self.session.command.execute_command.call_args[0][0]
        assert "'--staged'" in cmd
        assert "'--worktree'" in cmd

    @pytest.mark.asyncio
    async def test_restore_with_source(self):
        self.session.command.execute_command.return_value = _ok()
        await self.git.restore("/repo", ["file.txt"], source="HEAD~1")
        cmd = self.session.command.execute_command.call_args[0][0]
...
class TestAsyncGitPull(unittest.IsolatedAsyncioTestCase):
    """Tests for pull()."""
    ...
    @pytest.mark.asyncio
    async def test_pull_default(self):
        self.session.command.execute_command.return_value = _ok()
        await self.git.pull("/repo")
        cmd = self.session.command.execute_command.call_args[0][0]
        assert "'pull'" in cmd

    @pytest.mark.asyncio
    async def test_pull_with_remote_and_branch(self):
        self.session.command.execute_command.return_value = _ok()
        await self.git.pull("/repo", remote="origin", branch="main")
        cmd = self.session.command.execute_command.call_args[0][0]
        assert "'origin'" in cmd
        assert "'main'" in cmd
    ...
    @pytest.mark.asyncio
    async def test_pull_default_timeout(self):
        self.session.command.execute_command.return_value = _ok()
        await self.git.pull("/repo")
        kwargs = self.session.command.execute_command.call_args[1]
...
class TestAsyncGitConfig(unittest.IsolatedAsyncioTestCase):
    ...
    @pytest.mark.asyncio
    async def test_configure_user_global(self):
        self.session.command.execute_command.return_value = _ok()
        await self.git.configure_user("/repo", "Alice", "alice@x.com")
    ...
    @pytest.mark.asyncio
    async def test_configure_user_local(self):
        self.session.command.execute_command.return_value = _ok()
        await self.git.configure_user("/repo", "Alice", "alice@x.com", scope="local")
        cmd = self.session.command.execute_command.call_args_list[0][0][0]
        assert "'--local'" in cmd

    @pytest.mark.asyncio
    async def test_configure_user_name_failure(self):
        self.session.command.execute_command.return_value = _fail(stderr="error")
        with pytest.raises(GitError):
            await self.git.configure_user("/repo", "Alice", "alice@x.com")

    @pytest.mark.asyncio
    async def test_configure_user_email_failure(self):
    ...
    async def test_set_config_global(self):
        self.session.command.execute_command.return_value = _ok()
        await self.git.set_config("/repo", "core.autocrlf", "false")
        cmd = self.session.command.execute_command.call_args[0][0]
    ...
    @pytest.mark.asyncio
    async def test_set_config_local(self):
        self.session.command.execute_command.return_value = _ok()
        await self.git.set_config("/repo", "core.autocrlf", "false", scope="local")
        cmd = self.session.command.execute_command.call_args[0][0]
        assert "'--local'" in cmd
    ...
    async def test_set_config_failure(self):
        self.session.command.execute_command.return_value = _fail(stderr="error")
        with pytest.raises(GitError):
            await self.git.set_config("/repo", "k", "v")

    @pytest.mark.asyncio
    async def test_get_config_success(self):
        self.session.command.execute_command.return_value = _ok(stdout="false\n")
        val = await self.git.get_config("/repo", "core.autocrlf")
        assert val == "false"

    @pytest.mark.asyncio
    async def test_get_config_not_found_exit1_no_stderr(self):
        self.session.command.execute_command.return_value = _fail(
            stderr="", exit_code=1
        )
        val = await self.git.get_config("/repo", "nonexistent.key")
        assert val is None

    @pytest.mark.asyncio
    async def test_get_config_not_found_key_does_not_contain(self):
        self.session.command.execute_command.return_value = _fail(
            stderr="key does not contain a section", exit_code=1
        )
        val = await self.git.get_config("/repo", "bad-key")
        assert val is None

    @pytest.mark.asyncio
    async def test_get_config_other_error(self):
        self.session.command.execute_command.return_value = _fail(
            stderr="fatal: not in a git directory", exit_code=128
        )
        with pytest.raises(GitError):
            await self.git.get_config("/repo", "user.name")
