...
class TestAsyncSessionCallMcpTool(unittest.IsolatedAsyncioTestCase):
    ...
    @pytest.mark.asyncio
    async def test_call_mcp_tool_link_url_success_uses_session_tool_list(self):
    ...
    async def test_call_mcp_tool_link_url_fails_when_tool_missing_in_tool_list(self):
        ...
        with patch("httpx.AsyncClient") as mock_httpx_client, patch(
            "agentbay._async.session.CallMcpToolRequest"
        ) as MockCallMcpToolRequest, patch(
            "agentbay._async.session.extract_request_id"
