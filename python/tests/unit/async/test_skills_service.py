...
@pytest.mark.asyncio
async def test_agentbay_has_beta_skills():
    """AsyncAgentBay should have a 'beta_skills' attribute of type AsyncBetaSkillsService."""
    with patch("agentbay._async.agentbay._load_config") as mock_load_config, \
         patch("agentbay._async.agentbay.mcp_client") as mock_mcp_client:
        ...
        from agentbay._async.beta import AsyncBetaSkillsService
        assert isinstance(agent_bay.beta_skills, AsyncBetaSkillsService)


@pytest.mark.asyncio
async def test_get_metadata_parses_response():
    ...
    with patch("agentbay._async.agentbay._load_config") as mock_load_config, \
        ...
        agent_bay.client.get_skill_meta_data_async = AsyncMock(return_value=resp)
...
@pytest.mark.asyncio
async def test_get_metadata_passes_skill_names():
...
@pytest.mark.asyncio
async def test_get_metadata_handles_api_failure():
    ...
    with patch("agentbay._async.agentbay._load_config") as mock_load_config, \
        ...
        with pytest.raises(RuntimeError, match="GetSkillMetaData failed"):
            await agent_bay.beta_skills.get_metadata()


@pytest.mark.asyncio
async def test_get_metadata_skips_empty_names():
