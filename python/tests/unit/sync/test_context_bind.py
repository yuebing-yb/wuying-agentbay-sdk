...
class TestAsyncContextManagerBind(unittest.TestCase):
    ...
    @pytest.mark.sync
    def test_bind_no_contexts_returns_error(self):
        """bind() with no arguments should return an error."""
        result = self.context_manager.bind()
        self.assertFalse(result.success)
        self.assertEqual(result.error_message, "No contexts provided")

    @pytest.mark.sync
    def test_bind_single_context_success(self):
        ...
        self.mock_client.bind_contexts = MagicMock(return_value=mock_response)
        ...
        self.mock_client.bind_contexts.assert_called_once()

        call_args = self.mock_client.bind_contexts.call_args[0][0]
    ...
    @pytest.mark.sync
    def test_bind_api_failure(self):
    ...
    @pytest.mark.sync
    def test_bind_without_wait(self):
    ...
    @pytest.mark.sync
    def test_list_bindings_success(self):
        ...
        self.mock_client.describe_session_contexts = MagicMock(
            return_value=mock_response
        )

        result = self.context_manager.list_bindings()
    ...
    @pytest.mark.sync
    def test_list_bindings_api_failure(self):
    ...
    @pytest.mark.sync
    def test_list_bindings_null_data(self):
