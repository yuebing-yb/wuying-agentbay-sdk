...
class TestSessionPause(unittest.TestCase):
    ...
    def test_pause_success_immediate(self):
        ...
        self.agent_bay.client.pause_session_async = MagicMock(
        ...
        with patch("time.sleep", new_callable=MagicMock) as mock_sleep:
            ...
            result = self.session.beta_pause(timeout=10, poll_interval=1)
    ...
    def test_pause_api_error(self):
        ...
        with patch("time.sleep", new_callable=MagicMock) as mock_sleep:
            # Call the method
            result = self.session.beta_pause()
    ...
    def test_pause_client_exception(self):
        ...
        self.agent_bay.client.pause_session_async.side_effect = Exception(
            "Network error"
        )
    ...
    def test_pause_with_agent_bay_pause_method_session_exception(self):
        ...
        # Call the method with session object (AgentBay.pause is async)
        # We need to await it.
        result = self.agent_bay.beta_pause(self.session)
