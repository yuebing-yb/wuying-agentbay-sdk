...
from agentbay._sync.browser import BrowserError
...
class TestAsyncBrowser(unittest.TestCase):
    ...
    def test_act_async(self):
        ...
        response2 = MagicMock(success=True, data=json.dumps({"steps": [], "is_done": True, "success": True}))
    ...
    def test_observe_async(self):
        ...
        self.mock_session.call_mcp_tool.return_value = MagicMock(success=True, data=json.dumps([{"selector": "#search"}]))
    ...
    def test_extract_async(self):
        ...
        asyncio.run(self.browser.agent.extract(AsyncExtractOptions(instruction="Extract the title", schema=TestSchema), page))
        self.mock_session.call_mcp_tool.assert_called()
