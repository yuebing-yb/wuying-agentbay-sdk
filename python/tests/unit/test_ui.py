...
class TestUIApi(unittest.TestCase):
    ...
    def test_get_clickable_ui_elements_failure(self):
        self.ui._call_mcp_tool = MagicMock(
            side_effect=AgentBayError("Failed to get clickable UI elements")
        )
    ...
    def test_send_key_failure(self):
        self.ui._call_mcp_tool = MagicMock(
            side_effect=AgentBayError("Failed to send key")
        )
        with self.assertRaises(AgentBayError) as context:
    ...
    def test_swipe_failure(self):
        self.ui._call_mcp_tool = MagicMock(
            side_effect=AgentBayError("Failed to perform swipe")
        )
        with self.assertRaises(AgentBayError) as context:
            self.ui.swipe(100, 200, 300, 400, duration_ms=500)
    ...
    def test_click_failure(self):
        self.ui._call_mcp_tool = MagicMock(
            side_effect=AgentBayError("Failed to perform click")
        )
        with self.assertRaises(AgentBayError) as context:
            self.ui.click(150, 250, button="left")
    ...
    def test_input_text_failure(self):
        self.ui._call_mcp_tool = MagicMock(
            side_effect=AgentBayError("Failed to input text")
        )
        with self.assertRaises(AgentBayError) as context:
            self.ui.input_text("Hello, world!")
    ...
    def test_get_all_ui_elements_failure(self):
        self.ui._call_mcp_tool = MagicMock(
            side_effect=AgentBayError("Failed to get all UI elements")
        )
        with self.assertRaises(AgentBayError) as context:
    ...
    def test_screenshot_failure(self):
        ...
        with self.assertRaises(AgentBayError) as context:
            self.ui.screenshot()
        self.assertEqual(
            str(context.exception), "Error in response: Failed to take screenshot"
        )
