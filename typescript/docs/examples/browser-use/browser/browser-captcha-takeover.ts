/**
 * Example: browser-captcha-takeover.ts
...
let captchaSolvedSuccess = false;
...
const captchaDetectEvent = new EventEmitter();
...
async function main() {
  ...
  try {
    ...
    console.log("🔑 Captcha triggered, waiting for takeover notification...");

    const detected = await captchaDetectEvent.wait(maxCaptchaDetectTimeout * 1000);
    
    if (!detected) {
      console.log("⏰ No captcha detected within timeout, continuing...");
    } else if (shouldTakeover) {
      console.log("⏰ Captcha should takeover...");
