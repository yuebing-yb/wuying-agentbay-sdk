...
async function main() {
  ...
  try {
    ...
    try {
      const elements = await session.ui.getAllUIElements(2000); // 2 second timeout
    ...
    try {
      const clickableElements = await session.ui.getClickableUIElements(2000); // 2 second timeout
    ...
    try {
      await session.ui.sendKey(3); // 3 is HOME key code
    ...
    console.log('\n5. Inputting text...');
    try {
      await session.ui.inputText("Hello from AgentBay SDK!");
    ...
    const y = 960;
    try {
      await session.ui.click(x, y, "left");
    ...
    try {
      await session.ui.swipe(startX, startY, endX, endY, swipeDuration);
