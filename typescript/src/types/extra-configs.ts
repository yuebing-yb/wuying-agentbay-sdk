...
export function validateAppManagerRule(rule: AppManagerRule): void {
  ...
  if (rule.ruleType !== "White" && rule.ruleType !== "Black") {
    throw new Error(
      `Invalid ruleType: ${rule.ruleType}. Must be "White" or "Black"`
    );
  }
 ...
 */
export function validateMobileSimulateConfig(
  config: MobileSimulateConfig
): void {
  ...
  if (
    config.simulatePath !== undefined &&
    typeof config.simulatePath !== "string"
  ) {
    throw new Error("MobileSimulateConfig simulatePath must be a string");
  }

  if (
    config.simulatedContextId !== undefined &&
    typeof config.simulatedContextId !== "string"
  ) {
...
export function validateMobileExtraConfig(config: MobileExtraConfig): void {
  ...
  if (
    config.hideNavigationBar !== undefined &&
    typeof config.hideNavigationBar !== "boolean"
  ) {
