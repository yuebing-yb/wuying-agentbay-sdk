...
describe("Browser Context - Integration Tests", () => {
  ...
  beforeAll(async () => {
    ...
    // Create a context
    const contextResult: ContextResult = await agentBay.context.get(
      contextName,
      true
    );
  ...
  describe("Cookie Persistence", () => {
    it("should persist browser cookies across sessions with the same browser context (plain object)", async () => {
      ...
      try {
        ...
        log(
          `Step 1-2: Creating session with browser context ID: ${context.id}`
        );
        //test
        ...
        log(
          `Cookies found in first session: ${Object.keys(
            firstSessionCookieDict
          )}`
        );
        ...
        log(
          `First session deleted successfully (RequestID: ${deleteResult.requestId})`
        );
        ...
        log(
          `Step 6: Creating second session with same context ID: ${context.id}`
        );
        ...
        log(
          `Cookies found in second session (without opening page): ${Object.keys(
            secondSessionCookieDict
          )}`
        );
        ...
        const missingCookies = new Set(
          [...expectedCookieNames].filter((x) => !foundCookieNames.has(x))
        );
        if (missingCookies.size > 0) {
          throw new Error(
            `Missing expected test cookies in second session: ${Array.from(
              missingCookies
            )}`
          );
        }
        ...
        for (const cookieName of expectedCookieNames) {
          if (cookieName in secondSessionCookieDict) {
            const expectedValue = testCookies.find(
        ...
        log(
          `SUCCESS: All ${expectedCookieNames.size} test cookies persisted correctly!`
        );
      ...
      log(
        "Browser context manual cookie persistence test completed successfully!"
      );
    });
    ...
    it("should persist browser cookies across sessions with the same browser context (CreateSessionParams class)", async () => {
      ...
      try {
        ...
        log(
          `Step 1-2: Creating session with browser context ID using CreateSessionParams class: ${context.id}`
        );
        ...
        log(
          `First session created with class params, ID: ${session1.sessionId}`
        );
        ...
        log(
          `Step 6: Creating second session with same context ID using CreateSessionParams class: ${context.id}`
        );

        // Create new class instance for second session
        const params2 = new CreateSessionParamsClass({
          imageId: "browser_latest",
          browserContext,
        });
        ...
        log(
          `Second session created with class params, ID: ${session2.sessionId}`
        );
        ...
        log(
          `SUCCESS: All ${expectedCookieNames.size} test cookies persisted correctly with CreateSessionParams class!`
        );
        log(`Test cookies found: ${Array.from(expectedCookieNames)}`);
      } finally {
        // Clean up sessions if they weren't already cleaned up
      ...
      log(
        "Browser context cookie persistence test with CreateSessionParams class completed successfully!"
      );
    });
    ...
    it("should handle extension context syncs automatically with CreateSessionParams class", async () => {
      ...
      try {
        log(
          "Testing automatic extension context syncs merging with CreateSessionParams class"
        );
        ...
        const params = new CreateSessionParamsClass({
          imageId: "browser_latest",
          browserContext,
          labels: { testType: "extension-context-sync", framework: "jest" },
        });
        ...
        expect(params.labels).toEqual({
          testType: "extension-context-sync",
          framework: "jest",
        });
        ...
        if (!sessionResult.session) {
          throw new Error(
            "Failed to create session with CreateSessionParams class"
          );
        }
        ...
        log(
          "✓ Extension context syncs would be automatically merged if present"
        );
      } finally {
        // Clean up session
        if (session) {
      ...
      log(
        "Extension context syncs test with CreateSessionParams class completed successfully!"
      );
    });
  });
});
