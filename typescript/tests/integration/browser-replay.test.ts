  ...
  afterAll(async () => {
    ...
    if (playwrightBrowser) {
      try {
        ...
        log(
          `Warning: Error closing Playwright browser: ${
            error?.message || error
          }`
        );
      }
  ...
  test("should perform browser operations with recording", async () => {
    ...
    try {
      ...
      if (!chromium || typeof chromium.connectOverCDP !== "function") {
        throw new Error(
          "chromium.connectOverCDP is not available. Make sure playwright is installed: npm install playwright"
        );
      }

      log(
        `Attempting to connect to CDP endpoint: ${endpointUrl.substring(
          0,
          50
        )}...`
      );
      ...
      try {
        ...
        if (searchInput) {
          ...
          for (const btnSelector of buttonSelectors) {
            try {
              const searchButton = await page.waitForSelector(btnSelector, {
                timeout: 3000,
              });
        ...
        log(
          `Page interaction failed, but that's okay for recording test: ${
      ...
      log(
        `Browser operations encountered an error: ${error?.message || error}`
      );
      ...
      if (
        ...
        log(
          "The recording functionality is still being tested through session creation and browser initialization"
        );
      }
