...
async function saveScreenshotFromURL(
    ...
    client
      .get(screenshotURL, (response) => {
        if (response.statusCode !== 200) {
          reject(
            new Error(
              `Failed to download screenshot: HTTP ${response.statusCode}`
            )
          );
          return;
        }
      ...
      .on("error", (error) => {
        reject(
          new Error(`Failed to download screenshot from URL: ${error.message}`)
        );
      });
  });
}
...
describe("Computer Windows Integration Tests", () => {
  ...
  test("Computer Windows Integration Test", async () => {
    ...
    logInfo(
      "Step 1: Testing StartApp - Finding Calculator in installed apps..."
    );

    let startCmd = "",
      pname = ""; // Default fallback

    try {
      const installedApps = await session.computer.getInstalledApps(
        true,
        false,
        true
      );
    ...
    if (startResult.data.length > 0) {
      ...
      logInfo(
        `Started Calculator process: ${process.pname} (PID: ${process.pid})`
      );
    ...
    const activateResult = await session.computer.activateWindow(
      calculatorWindowID
    );

    expect(activateResult.success).toBe(true);
    logInfo(
      `Successfully activated Calculator window (ID: ${calculatorWindowID})`
    );
    ...
    if (activeWindowResult.window) {
      logInfo(
        `Active window: ${activeWindowResult.window.title} (ID: ${activeWindowResult.window.windowId})`
      );
      // The active window should be Calculator (though title might vary by locale)
      const isCalculator =
        activeWindowResult.window.title.toLowerCase().includes("calculator") ||
    ...
    // Maximize window
    const maximizeResult = await session.computer.maximizeWindow(
      calculatorWindowID
    );
    ...
    if (screenshot.success && screenshot.data) {
      ...
      logInfo(
        `Screenshot failed or returned empty data: ${screenshot.errorMessage}`
      );
    }
    ...
    // Minimize window
    const minimizeResult = await session.computer.minimizeWindow(
      calculatorWindowID
    );
    ...
    // Restore window
    const restoreResult = await session.computer.restoreWindow(
      calculatorWindowID
    );
    ...
    const resizeResult = await session.computer.resizeWindow(
      calculatorWindowID,
      600,
      400
    );
    ...
    const fullscreenResult = await session.computer.fullscreenWindow(
      calculatorWindowID
    );
    ...
    const calculatorApp = visibleAppsResult.data.find(
      (app: any) =>
    ...
    if (calculatorApp) {
      logInfo(
        `Calculator found in visible apps: ${calculatorApp.pname} (PID: ${calculatorApp.pid})`
      );
      expect(calculatorApp.pname).toBeDefined();
      expect(calculatorApp.pid).toBeGreaterThan(0);
    } else {
      logInfo(
        "Note: Calculator not found in visible apps (may have been closed or minimized)"
      );
    }
    ...
    try {
      const closeResult = await session.computer.closeWindow(
        calculatorWindowID
      );
