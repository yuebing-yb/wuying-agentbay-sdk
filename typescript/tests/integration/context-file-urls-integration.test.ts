...
describe("Context File URLs Integration", () => {
  ...
  it("should get upload and download URLs, verify content and list/delete with retries", async () => {
    ...
    // 1) Get upload URL
    const uploadURLRes = await agentBay.context.getFileUploadUrl(
      contextId,
      testPath
    );
    ...
    log(
      `Upload URL: ${uploadURLRes.url.slice(0, 80)}... (RequestID: ${
        uploadURLRes.requestId
      })`
    );

    // 2) Upload content using the presigned URL
    const uploadContent = Buffer.from(
      `agentbay integration upload test at ${Date.now()}\n`,
      "utf-8"
    );
    const putResp = await fetch(uploadURLRes.url, {
      method: "PUT",
      body: uploadContent,
    });
    ...
    log(
      `Uploaded ${uploadContent.length} bytes, status=${putResp.status}, ETag=${etag}`
    );
    ...
    for (let attempt = 1; attempt <= 5; attempt++) {
      try {
        dlURLRes = await agentBay.context.getFileDownloadUrl(
          contextId,
          testPath
        );
        if (dlURLRes.success) {
        ...
        log(
          `Download URL attempt ${attempt} failed: ${
            dlURLRes.errorMessage || "Unknown error"
          }`
        );
      } catch (error: any) {
    ...
    log(
      `Download URL: ${dlURLRes!.url.slice(0, 80)}... (RequestID: ${
        dlURLRes!.requestId
      })`
    );
    ...
    if (lastListRes && chosenParent) {
      const total =
        typeof lastListRes.count === "number"
          ? lastListRes.count
          : lastListRes.entries?.length || 0;
      log(
        `List files: checked parent=${chosenParent}, total=${total}, contains=${present}`
      );
    } else {
    ...
    if (lastListRes) {
      const prev =
        typeof lastListRes.count === "number"
          ...
          : lastListRes.entries?.length || 0;
      log(
        `List files: ${fileName} absent after delete (listing availability: ${prev})`
      );
    }
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
      try {
        const postDL = await agentBay.context.getFileDownloadUrl(
          contextId,
          testPath
        );
        if (postDL && postDL.success && postDL.url && postDL.url.length > 0) {
          ...
          log(
            `Post-delete download attempt ${attempt} status: ${postResp.status}`
          );
          ...
          log(
            `Post-delete attempt ${attempt}: download URL not available, treated as deleted`
          );
          postDeleteVerified = true;
          break;
        }
        ...
        if (error.message && error.message.includes("file not exist")) {
          log(
            `Post-delete attempt ${attempt}: file confirmed deleted (${error.message})`
          );
          postDeleteVerified = true;
          break;
        }
