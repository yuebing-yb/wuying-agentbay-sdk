...
describe("ContextPaginationIntegration", () => {
  ...
  beforeAll(async () => {
    ...
    for (let i = 0; i < 15; i++) {
      ...
      if (contextResult.success && contextResult.context) {
        ...
        log(
          `Created context: ${contextName} (ID: ${contextResult.context.id})`
        );
      } else {
    ...
    for (const contextName of contextNames) {
      try {
        ...
        if (contextResult.success && contextResult.context) {
          ...
          log(
            `Deleted context: ${contextName} (ID: ${contextResult.context.id})`
          );
        }
      } catch (e) {
  ...
  it("should list contexts with default pagination (first page)", async () => {
    ...
    log(
      `First page: Got ${listResult.contexts.length} contexts (RequestID: ${listResult.requestId})`
    );
    log(
      `NextToken: ${listResult.nextToken}, MaxResults: ${listResult.maxResults}, TotalCount: ${listResult.totalCount}`
    );
  });
  ...
  it("should list contexts with custom page size", async () => {
    ...
    log(
      `Custom page size: Got ${listResult.contexts.length} contexts (RequestID: ${listResult.requestId})`
    );
    log(
      `NextToken: ${listResult.nextToken}, MaxResults: ${listResult.maxResults}, TotalCount: ${listResult.totalCount}`
    );
  ...
  it("should get second page using NextToken", async () => {
    ...
    if (firstPageResult.nextToken) {
      ...
      log(
        `Second page: Got ${secondPageResult.contexts.length} contexts (RequestID: ${secondPageResult.requestId})`
      );
      log(
        `NextToken: ${secondPageResult.nextToken}, MaxResults: ${secondPageResult.maxResults}, TotalCount: ${secondPageResult.totalCount}`
      );
    } else {
  ...
  it("should list contexts with larger page size", async () => {
    ...
    log(
      `Larger page size: Got ${listResult.contexts.length} contexts (RequestID: ${listResult.requestId})`
    );
    log(
      `NextToken: ${listResult.nextToken}, MaxResults: ${listResult.maxResults}, TotalCount: ${listResult.totalCount}`
    );
  ...
  it("should list contexts with nil parameters (should use defaults)", async () => {
    ...
    log(
      `Nil parameters: Got ${listResult.contexts.length} contexts (RequestID: ${listResult.requestId})`
    );
