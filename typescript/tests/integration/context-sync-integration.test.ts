...
describe("ContextSyncIntegration", () => {
  ...
  beforeAll(async () => {
    ...
    const sessionParams = {
      contextSync: [newContextSync(contextId, "/home/wuying", newSyncPolicy())],
  ...
  afterAll(async () => {
    ...
    try {
      if (sessionId) {
        ...
        const sessionIdFound = sessionListResult.sessionIds.find(
  ...
  it("should sync context and return info", async () => {
    ...
    // Log the context status data
    log(
      `Context status data after sync, count: ${contextInfo.contextStatusData.length}`
    );
    for (let i = 0; i < contextInfo.contextStatusData.length; i++) {
      const data = contextInfo.contextStatusData[i];
      log(`Status data ${i}:`);
  ...
  it("should get context info with parameters", async () => {
    ...
    log(
      `Filtered context status data count: ${contextInfo.contextStatusData.length}`
    );
    for (let i = 0; i < contextInfo.contextStatusData.length; i++) {
      const data = contextInfo.contextStatusData[i];
      log(`Status data ${i}:`);
  ...
  it("should test context sync persistence with retry", async () => {
    ...
    const sessionParams = {
      ...
      contextSync: [newContextSync(contextIdLocal, syncPath, newSyncPolicy())],
    };
    ...
    for (let i = 0; i < 20; i++) {
      ...
      log(
        `No context status data available yet (attempt ${
          i + 1
    ...
    for (let i = 0; i < 20; i++) {
      ...
      log(
        `No upload status found yet (attempt ${i + 1}), retrying in 1 second...`
      );
      await wait(1000);
    }
    ...
    for (let i = 0; i < 20; i++) {
      ...
      log(
        `No download status found yet (attempt ${
          i + 1
        }), retrying in 1 second...`
      );
      await wait(1000);
