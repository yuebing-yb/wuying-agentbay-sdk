...
async function validateExtensionServiceIntegration(
  ...
  try {
    ...
    results.serviceIntegration =
      results.fileSystemCheck && results.extensionServiceValidation;

    log(
      `Validation results: fileSystem=${results.fileSystemCheck}, service=${results.extensionServiceValidation}, integration=${results.serviceIntegration}`
    );
  ...
  const manifests = [
    {
      ...
      version: "1.0.0",
      description:
        "A test ad blocking extension with background service worker",
...
async function createExtensionZip(
  name: string,
  manifest: any,
  useDirectoryStructure: boolean = false
): Promise<string> {
  ...
  let manifestPath: string,
    ...
    popupJsPath: string,
    backgroundJsPath: string,
    contentJsPath: string;
...
<body>
    ...
    <div class="directory-info">
        <p><strong>Structure:</strong> ${
          useDirectoryStructure ? "Directory-based" : "Flat structure"
        }</p>
            ...
            chrome.storage.local.set(testData, () => {
                if (chrome.runtime.lastError) {
                    ...
                    resultDiv.innerHTML = \`
                        ...
                        <p>📁 Structure: ${
                          useDirectoryStructure ? "Directory-based" : "Flat"
                        }</p>
                    \`;
                }
                ...
                window.__EXTENSION_STORAGE_TEST__ = {
                    ...
                    extensionName: '${manifest.name}',
                    directoryBased: '${
                      useDirectoryStructure ? "true" : "false"
                    }',
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    ...
    switch (request.action) {
        case 'showExtensionInfo':
            ...
            \`;
            notification.textContent = \`Extension ${
              manifest.name
            } is active!\`;
...
log('Content script fully initialized for ${
    manifest.name
  ...
  try {
    ...
    try {
      if (process.platform === "win32") {
        ...
        execSync(`cd "${tempDir}" && zip -r "${name}.zip" *`, {
          stdio: "pipe",
        });
      }
      ...
      if (fs.existsSync(zipPath)) {
        ...
        if (stats.size > 0) {
          log(
            `✅ Successfully created ZIP file using system zip: ${zipPath} (${stats.size} bytes)`
          );
          return zipPath;
        }
      }
      ...
      log(
        `System zip command not available or failed: ${
    ...
    if (fs.existsSync(tarPath)) {
      ...
      if (stats.size > 0) {
        log(
          `✅ Successfully created TAR.GZ file: ${tarPath} (${stats.size} bytes)`
        );
      ...
      const simpleZipContent = JSON.stringify(
        {
          ...
          files: {
            ...
            "popup.html": fs.readFileSync(
              path.join(tempDir, "popup.html"),
              "utf8"
            ),
            "popup.js": fs.readFileSync(path.join(tempDir, "popup.js"), "utf8"),
            "background.js": fs.readFileSync(
              path.join(tempDir, "background.js"),
              "utf8"
            ),
            "content.js": fs.readFileSync(
              path.join(tempDir, "content.js"),
              "utf8"
            ),
          },
        },
...
describe("Extension Service Integration Tests", () => {
  ...
  beforeAll(async () => {
    ...
    if (!apiKey || apiKey === "akm-xxx" || process.env.CI) {
      throw new Error(
        "Skipping integration test: No valid API key or running in CI. Set AGENTBAY_API_KEY environment variable to run integration tests."
      );
    }
    ...
    try {
      ...
      const browserContextResult: ContextResult = await agentBay.context.get(
        browserContextName,
        true
      );
      ...
      log(
        `Created browser context: ${browserContextName} (ID: ${browserContextId})`
      );
      ...
      for (let i = 0; i < sampleExtensions.length; i++) {
        ...
        try {
          log(
            `Uploading extension ${i + 1}/${
              sampleExtensions.length
          ...
          log(
            `❌ Failed to upload extension ${path.basename(extPath)}:`,
            uploadError
          );
      ...
      log(
        `✅ Pre-uploaded ${uploadedExtensions.length} extensions for testing`
      );
    } catch (setupError) {
    ...
    it("should perform complete CRUD workflow using public API methods", async () => {
      ...
      try {
        if (uploadedExtensions.length >= 2) {
          ...
          log(
            `Using ${testExtensions.length} pre-uploaded extensions for testing`
          );
        } else {
          // Upload extensions specifically for this test
          log(
            "Pre-uploaded extensions not available, creating test extensions..."
          );
          shouldCleanup = true;
          ...
          for (let i = 0; i < Math.min(2, sampleExtensions.length); i++) {
            try {
              ...
              const extension = await extensionsService.create(
                sampleExtensions[i]
              );
              ...
              if (
                ...
                createError.message.includes("403 Forbidden")
              ) {
                log("\n🔑 Upload Authentication Failed:");
                log(
                  "   This test requires a valid AGENTBAY_API_KEY with file upload permissions."
                );
                log(
                ...
                log(`   Current API key: ${apiKey.substring(0, 8)}...`);
                throw new Error(
                  `Authentication failed during extension upload: ${createError.message}`
                );
              }
        ...
        if (testExtensions.length < 2) {
          throw new Error(
            `Insufficient test extensions: need 2, have ${testExtensions.length}`
          );
        }
        ...
        expect(extensionList.length).toBeGreaterThanOrEqual(
          testExtensions.length
        );
  ...
  describe("Extension Delete Operation", () => {
    it("should delete extension using public API method", async () => {
      ...
      const extensionList = await extensionsService.list();
      const deletedIds = extensionList.filter(
        (ext) => ext.id === testExtension.id
      );
  ...
  describe("Extension Create Operation", () => {
    it("should create extension using public API method", async () => {
      ...
      const extensionList = await extensionsService.list();
      const createdIds = extensionList.filter(
  ...
  describe("Extension Service Integration", () => {
    it("should perform comprehensive extension service integration", async () => {
      ...
      if (uploadedExtensions.length === 0) {
        throw new Error(
          "No uploaded extensions available for integration testing. Extension upload may have failed during setup."
        );
      }
      ...
      if (extensionIds.length === 0) {
        throw new Error(
          "No extension IDs available for testing. Check extension upload process."
        );
      }
      ...
      try {
        ...
        log("\nPhase 1: Extension Synchronization");
        const lsResult_before = await session.command.executeCommand(
          "ls -la /tmp/extensions/"
        );
        ...
        log(
          `    ✅  lsResult_before Extensions directory exists: /tmp/extensions/`
        );
        log(`    Directory content:\n${lsResult_before.output}`);

        // Phase 2: Browser Initialization with Extensions
        log("\nPhase 2: Browser Initialization with Extensions");
        const browserInitialized = await initializeBrowserWithExtensions(
          session
        );
        ...
        log("\nPhase 3: Extension Service Validation");
        const validationResults = await validateExtensionServiceIntegration(
          session,
          extensionIds
        );
        ...
        log(
          `   - Extension Service Validation: ${validationResults.extensionServiceValidation}`
        );
        log(
          `   - Context Sync Validation: ${validationResults.contextSyncValidation}`
        );
        log(
          `   - Service Integration: ${validationResults.serviceIntegration}`
        );

        // Phase 4: Extension File Verification
        log("\nPhase 4: Extension File Verification");
        const fileVerificationResult = await verifyExtensionFiles(
          session,
          extensionIds
        );
  ...
  async function createSessionWithExtensions(
    extensionIds: string[]
  ): Promise<Session> {
    ...
    if (!extensionIds || extensionIds.length === 0) {
      throw new Error(
        "Cannot create session: extensionIds array is empty. Ensure extensions are uploaded successfully before creating session."
      );
    }

    // Validate that all extension IDs are valid strings
    const invalidIds = extensionIds.filter(
      (id) => !id || typeof id !== "string" || id.trim().length === 0
    );
    if (invalidIds.length > 0) {
      throw new Error(
        `Invalid extension IDs found: ${JSON.stringify(
          invalidIds
        )}. All extension IDs must be non-empty strings.`
      );
    }
    ...
    const extensionOption =
      extensionsService.createExtensionOption(extensionIds);
  ...
  async function initializeBrowserWithExtensions(
    session: Session
  ): Promise<boolean> {
    try {
      ...
      log(
        `  ✅ Browser initialized successfully with extension_path: /tmp/extensions/`
      );
  ...
  async function verifyExtensionFiles(
    ...
    try {
      ...
      for (const extId of extensionIds) {
        ...
        const folderCheck = await session.command.executeCommand(
          `test -d /tmp/extensions/${baseExtId}`
        );
        if (folderCheck.success) {
          log(`    ✅ Found extension folder: ${baseExtId}`);
          const manifestCheck = await session.command.executeCommand(
            `test -f /tmp/extensions/${baseExtId}/manifest.json`
          );
      ...
      log(
        `    File verification result: ${foundExtensions}/${extensionIds.length} extensions found`
      );
      return success;
    } catch (error) {
  ...
  async function createSessionWithExtensionsUsingClass(
    extensionIds: string[]
  ): Promise<Session> {
    log(
      `  📦 Creating session with extension IDs using CreateSessionParams class: ${extensionIds}`
    );
    ...
    log(
      "\nCreating session with browser context using CreateSessionParams class...",
      sessionParams
    );

    const sessionResult: SessionResult = await agentBay.create(sessionParams);
    log(
      `  ✅ Created session using CreateSessionParams class: ${sessionResult.session.sessionId}`
    );
    expect(sessionResult.success).toBe(true);
    const session = sessionResult.session;
    expect(session).toBeDefined();
    ...
    const sessionId = session.sessionId;
    log(
      `  ✅ Session created with extension support using CreateSessionParams class: ${sessionId}`
    );
    log(`  ✅ Extension IDs configured: ${extensionIds}`);
    return session;
  }
  ...
  async function cleanupSession(session: Session): Promise<void> {
    ...
    try {
      ...
      if (deleteResult.success) {
        ...
        log(
          `  ⚠️ Failed to delete session: ${
            deleteResult.errorMessage || "Unknown error"
          }`
        );
      }
    } catch (e) {
