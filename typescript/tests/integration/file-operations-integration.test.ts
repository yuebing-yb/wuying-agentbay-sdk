...
describe("File Operations Integration Tests", () => {
  ...
  describe("File System CRUD Operations", () => {
    it("should perform complete file system operations workflow", async () => {
      ...
      const writeLargeResult = await fileSystem.writeFile(
        "/tmp/user/large_context.txt",
        largeContent
      );
      ...
      const largeReadResult = await fileSystem.readFile(
        "/tmp/user/large_context.txt"
      );
      ...
      // Step 12: File moving
      const moveResult = await fileSystem.moveFile(
        "/tmp/user/test.txt",
        "/tmp/test.txt"
      );
      ...
      const searchLargeResults = await fileSystem.searchFiles(
        "/tmp",
        "large_context.txt"
      );
      // Verify large_context.txt is still there
      const largeFile = searchLargeResults.matches.find((file) =>
        file.includes("large_context.txt")
      );
    ...
    it("should handle edge cases and special conditions", async () => {
      ...
      const writeEmptyResult = await fileSystem.writeFile(
        "/tmp/user/empty.txt",
        "",
        "create_new"
      );
      ...
      const specialContent =
        "Content with Chinese and special chars: !@#$%^&*()";

      const writeSpecialResult = await fileSystem.writeFile(
        specialFileName,
        specialContent,
        "create_new"
      );
      ...
      const createDeepDirResult = await fileSystem.createDirectory(
        "/tmp/user/deep/nested/structure"
      );
      expect(createDeepDirResult.success).toBe(true);

      const writeDeepResult = await fileSystem.writeFile(
        "/tmp/user/deep/nested/structure/deep.txt",
        "deep content",
        "create_new"
      );
      expect(writeDeepResult.success).toBe(true);

      const deepReadResult = await fileSystem.readFile(
        "/tmp/user/deep/nested/structure/deep.txt"
      );
  ...
  describe("Batch File Operations", () => {
    ...
    it.only("should search files correctly", async () => {
      ...
      await session.fileSystem.writeFile(
        "/tmp/search_test/test1.txt",
        "content",
        "overwrite"
      );
      await session.fileSystem.writeFile(
        "/tmp/search_test/test2.log",
        "log",
        "overwrite"
      );

      const result1 = await session.fileSystem.searchFiles(
        "/tmp/search_test",
        "test1.txt"
      );
      expect(result1.success).toBe(true);
      expect(result1.matches).toBeDefined();
      const result2 = await session.fileSystem.searchFiles(
        "/tmp/search_test",
        "",
        ["test2.log"]
      );
