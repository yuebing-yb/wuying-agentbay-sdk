...
describe("Mobile Simulate Integration Tests", () => {
  ...
  describe("Mobile Simulate for Model A", () => {
    test("should simulate Model A mobile device", async () => {
      ...
      const mobileInfoPath = path.join(
        __dirname,
        "..",
        "..",
        "..",
      ...
      try {
        ...
        const cmdResult = await (session as any).command.executeCommand(
  ...
  describe("Mobile Simulate Persistence", () => {
    test("should use persistent mobile simulate context", async () => {
      ...
      try {
        ...
        expect(deleteResult.success).toBe(true);
        log(
          `Session 1 deleted successfully (RequestID: ${deleteResult.requestId})`
        );
      }

      // For Session 2
      log(
        `Using a persistent mobile simulate context id: ${mobileSimContextID}`
      );
      ...
      try {
        ...
        const cmdResult2 = await (session2 as any).command.executeCommand(
          "getprop ro.product.model"
        );
        ...
        expect(deleteResult.success).toBe(true);
        log(
          `Session 2 deleted successfully (RequestID: ${deleteResult.requestId})`
        );
      }
    }, 60000); // 60 second timeout
  });
