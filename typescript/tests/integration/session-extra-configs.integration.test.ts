...
describe("Session Extra Configurations Integration", () => {
  ...
  describe("Mobile Session Creation with Extra Configurations", () => {
    it("should create mobile session with comprehensive extra configurations", async () => {
      ...
      console.log(
        "Step 1: Creating mobile configuration with extra configs..."
      );
      const appRule: AppManagerRule = {
        ruleType: "White",
        appPackageNameList: ["com.android.settings", "com.example.test.app"],
      };
      ...
      const mobileConfig: MobileExtraConfig = {
        ...
        uninstallBlacklist: [
          // New feature: uninstall protection
      ...
      console.log(
        `Mobile config created: lockResolution=${mobileConfig.lockResolution}, ` +
      ...
      console.log(
        `Session params: imageId=${params.imageId}, labels=${JSON.stringify(
          params.labels
        )}`
      );
      ...
      console.log(
        "Step 2: Creating mobile session with extra configurations..."
      );
      ...
      console.log(
        `✓ Mobile session created successfully with ID: ${session.sessionId} (RequestID: ${createResult.requestId})`
      );
      ...
      try {
        ...
        if (infoResult.success) {
          ...
          expect(
            resourceUrl.includes("android") || resourceUrl.includes("mobile")
          ).toBe(true);
          console.log(
            `✓ Mobile environment verified (RequestID: ${infoResult.requestId})`
          );
        } else {
          console.log(
            `⚠ Failed to get session info: ${infoResult.errorMessage}`
          );
        }
        ...
        if (labelsResult.success) {
          ...
          console.log(
            `✓ Labels verified: ${JSON.stringify(labels)} (RequestID: ${
              labelsResult.requestId
            })`
          );
        } else {
          console.log(
            `⚠ Failed to get session labels: ${labelsResult.errorMessage}`
          );
        }
        ...
        if (screenshotResult.success) {
          ...
          console.log(
            `✓ Mobile screenshot working (RequestID: ${screenshotResult.requestId})`
          );
        } else {
          console.log(
            `⚠ Mobile screenshot failed: ${screenshotResult.errorMessage}`
          );
        }
        ...
        try {
          ...
          const navBarResult = await session.mobile.setNavigationBarVisibility(
            true
          );
          const uninstallResult = await session.mobile.setUninstallBlacklist([
            "com.android.systemui",
          ]);
        ...
        if (deleteResult.success) {
          ...
          console.log(
            `⚠ Failed to delete session: ${deleteResult.errorMessage}`
          );
        }
      ...
      console.log(
        "✓ Mobile extra configs integration test completed successfully"
      );
    });
    ...
    it("should create mobile session with blacklist configuration", async () => {
      ...
      const appRule: AppManagerRule = {
        ruleType: "Black",
        appPackageNameList: ["com.malware.suspicious", "com.unwanted.adware"],
      };
      ...
      const session = createResult.session!;
      console.log(
        `✓ Mobile session with blacklist created: ${session.sessionId}`
      );

      try {
        // Test blacklist configuration
        const blacklistResult = await session.mobile.setAppBlacklist([
          "com.test.blocked",
        ]);
        console.log(
          `Blacklist configuration result: ${blacklistResult.success}`
        );
      } finally {
        const deleteResult = await agentBay.delete(session);
        expect(deleteResult.success).toBe(true);
