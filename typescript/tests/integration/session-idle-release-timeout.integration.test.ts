...
describe("Session idle release timeout (integration)", () => {
  it(
    "should release session between timeout and timeout+60s",
    async () => {
      ...
      try {
        ...
        while (Date.now() < timeoutDeadline) {
          ...
          if (!status.success && status.code === "InvalidMcpSession.NotFound") {
            throw new Error(
              `Session released too early: NotFound before ${idleReleaseTimeoutSeconds}s`
            );
          }
          if (
            status.success &&
            ...
            throw new Error(
              `Session released too early: status=${status.status} before ${idleReleaseTimeoutSeconds}s`
            );
          }
        ...
        while (Date.now() < deadline) {
          ...
          if (!status.success && status.code === "InvalidMcpSession.NotFound") {
            ...
            expect(elapsed).toBeLessThanOrEqual(
              idleReleaseTimeoutSeconds + maxOverSeconds
            );
            log(
              `✅ Session released: NotFound, elapsed=${elapsed.toFixed(2)}s`
            );
            return;
          }
        ...
        log(
          `Create request started at: ${new Date(createStart).toISOString()}`
        );
      }
    },
    2 * 60 * 1000
