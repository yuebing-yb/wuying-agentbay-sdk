...
function isReleased(status: any): boolean {
  ...
  if (status.success === false && status.code === "InvalidMcpSession.NotFound") {
    return true;
  }
...
describe("Session keepAlive (integration)", () => {
  it("should keep refreshed session alive longer than control session", async () => {
    ...
    try {
      ...
      const deadline = start + (idleReleaseTimeoutSeconds + maxOverSeconds) * 1000;
      while (Date.now() < deadline) {
        ...
        if (isReleased(controlStatus)) {
          ...
          log(`✅ Control released while refreshed alive, elapsed=${elapsed.toFixed(2)}s`);
          return;
        }
