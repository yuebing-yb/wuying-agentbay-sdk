...
describe("Session Labels", () => {
  ...
  test("setLabels empty object handling test", async () => {
    ...
    expect(emptyResult.errorMessage).toBe(
  ...
  test(" setLabels empty keys/values handling test", async () => {
    ...
    expect(emptyKeyResult.errorMessage).toBe(
    ...
    expect(emptyValueResult.errorMessage).toBe(
    ...
    expect(nullValueResult.errorMessage).toBe(
      "Label values cannot be empty Please provide valid values."
    );

    // Test undefined value
    const undefinedValueResult = await session.setLabels({
      key: undefined as any,
    });
    expect(undefinedValueResult.success).toBe(false);
    expect(undefinedValueResult.errorMessage).toBe(
      "Label values cannot be empty Please provide valid values."
    );

    log(
      `TC-ERROR-003 setLabels empty keys/values: All empty keys and values correctly rejected`
    );
  });
  test("setLabels mixed invalid parameters test", async () => {
    ...
    expect(mixedEmptyKeyResult.errorMessage).toBe(
      "Label keys cannot be empty Please provide valid keys."
    );
    ...
    expect(mixedEmptyValueResult.errorMessage).toBe(
      "Label values cannot be empty Please provide valid values."
    );
    ...
    expect(multipleInvalidResult.errorMessage).toBe(
      "Label keys cannot be empty Please provide valid keys."
    );

    log(
      `setLabels mixed invalid parameters: Mixed invalid parameters correctly handled with proper priority`
    );
  });
  ...
  describe("listByLabels()", () => {
    it.only("should list sessions filtered by labels", async () => {
      ...
      try {
        ...
        log(`List RequestId: ${listResponse.requestId || "undefined"}`);
