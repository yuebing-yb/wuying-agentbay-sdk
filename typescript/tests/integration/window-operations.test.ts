      ...
      if (session.window && session.Application) {
        ...
        if (processes && processes.length > 0) {
          ...
          const cleanupTerminal = async () => {
            for (const process of processes) {
              if (process.pid > 0) {
                ...
                try {
                  const stopResponse = await session.Application.stopAppByPID(process.pid);
                  log(`Successfully stopped Terminal process (PID: ${process.pid})`);
                  log(`Stop App RequestId: ${stopResponse.requestId || 'undefined'}`);
                } catch (stopError) {
          ...
          try {
            ...
            log(`List Root Windows RequestId: ${rootWindowsResponse.requestId || 'undefined'}`);
            ...
            try {
              const restoreResponse = await session.window.restoreWindow(windowId);
            ...
            try {
              const resizeResponse = await session.window.resizeWindow(windowId, 800, 600);
            ...
            try {
              const minimizeResponse = await session.window.minimizeWindow(windowId);
            ...
            try {
              const maximizeResponse = await session.window.maximizeWindow(windowId);
            ...
            try {
              const terminalStartResponse = await session.Application.startApp(terminalCmd, '');
              ...
              log(`Start Terminal RequestId: ${terminalStartResponse.requestId || 'undefined'}`);
                ...
                for (const process of terminalProcesses) {
                  if (process.pid > 0) {
                    ...
                    try {
                      const stopResponse = await session.Application.stopAppByPID(process.pid);
                      log(`Successfully stopped terminal process (PID: ${process.pid})`);
                      log(`Stop Terminal RequestId: ${stopResponse.requestId || 'undefined'}`);
                    } catch (stopError) {
                      log(`Warning: Failed to stop terminal process: ${stopError}`);
                    }
                  }
              ...
              try {
                ...
                try {
                  const activateResponse = await session.window.activateWindow(windowId);
                ...
                try {
                  const fullscreenResponse = await session.window.fullscreenWindow(windowId);
                  log('Window fullscreened successfully');
                  log(`Fullscreen Window RequestId: ${fullscreenResponse.requestId || 'undefined'}`);
                } catch (error) {
                ...
                try {
                  const closeResponse = await session.window.closeWindow(windowId);
                  log('Window closed successfully');
                  log(`Close Window RequestId: ${closeResponse.requestId || 'undefined'}`);
                } catch (error) {
