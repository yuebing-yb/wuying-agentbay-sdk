...
describe('Agent target resolution', () => {
  const mockSession = {
    ...
    callMcpTool: async () => ({ success: true, data: '{}', errorMessage: '', requestId: '' }),
    mcpTools: [],
  };
...
describe('hasStreamingParams', () => {
  ...
  test('returns true when onReasoning is provided', () => {
    const agent = new MobileUseAgent(mockSession);
    expect((agent as any).hasStreamingParams({ onReasoning: () => {} })).toBe(true);
  });

  test('returns true when onContent is provided', () => {
    const agent = new MobileUseAgent(mockSession);
    expect((agent as any).hasStreamingParams({ onContent: () => {} })).toBe(true);
  });

  test('returns true when onToolCall is provided', () => {
    const agent = new MobileUseAgent(mockSession);
    expect((agent as any).hasStreamingParams({ onToolCall: () => {} })).toBe(true);
  });

  test('returns true when onToolResult is provided', () => {
    const agent = new MobileUseAgent(mockSession);
    expect((agent as any).hasStreamingParams({ onToolResult: () => {} })).toBe(true);
  });
...
describe('MobileUseAgent executeTaskAndWait routing', () => {
  test('should route to WS when streaming options provided', async () => {
  ...
  test('should route to WS when options passed as 3rd arg', async () => {
    let wsStreamCalled = false;
    const mockSession = {
      getAPIKey: () => 'test_key',
      getSessionId: () => 'sess_test',
  ...
  test('should use HTTP polling when no streaming options', async () => {
    ...
    const mockSession = {
      ...
      callMcpTool: async () => {
        ...
        return {
          success: true,
          data: JSON.stringify({ task_id: 'task_123', status: 'completed', result: 'done' }),
          errorMessage: '',
          requestId: 'req_test',
        };
      },
...
describe('Streaming event dispatch', () => {
  test('should dispatch events to typed callbacks', async () => {
    ...
    let onEventHandler: ((invocationId: string, data: any) => void) | null = null;
    ...
    const mockSession = {
      ...
      getWsClient: async () => ({
        callStream: async (params: any) => {
          ...
          return {
            waitEnd: () => new Promise<any>((resolve) => {
              setTimeout(() => {
                if (onEventHandler) {
                  onEventHandler('inv_test', { eventType: 'reasoning', seq: 1, round: 1, content: 'thinking' });
                  onEventHandler('inv_test', { eventType: 'content', seq: 2, round: 1, content: 'hello' });
                  onEventHandler('inv_test', { eventType: 'tool_call', seq: 3, round: 1, toolCallId: 'tc1', toolName: 'click', args: { x: 100 } });
                  onEventHandler('inv_test', { eventType: 'tool_result', seq: 4, round: 1, toolCallId: 'tc1', toolName: 'click', result: { output: 'ok' } });
                }
    ...
    const result = await agent.executeTaskAndWait('Test task', 60, {
      ...
      onToolResult: (e) => toolResultEvents.push(e),
    });
        ...
        callStream: async (params: any) => {
          ...
          return {
            waitEnd: () => new Promise<any>((resolve) => {
              setTimeout(() => {
                if (onEventHandler) {
                  onEventHandler('inv_test', { eventType: 'content', seq: 1, round: 1, content: 'Hello' });
                  onEventHandler('inv_test', { eventType: 'content', seq: 2, round: 1, content: ' World' });
                }
