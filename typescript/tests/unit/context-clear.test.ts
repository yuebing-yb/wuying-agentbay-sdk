...
describe("Context Clear Operations", () => {
  ...
  describe("clearAsync", () => {
    ...
    it("should throw APIError on network errors", async () => {
      ...
      await expect(
        mockContextService.clearAsync("context-123")
  ...
  describe("getClearStatus", () => {
    ...
    it("should handle API errors", async () => {
      ...
      const result = await mockContextService.getClearStatus(
        "invalid-context-id"
      );
  ...
  describe("clear", () => {
    it("should complete clearing successfully", async () => {
      ...
      mockClient.getContext
        .onFirstCall()
        .resolves(statusResponses[0])
        .onSecondCall()
        .resolves(statusResponses[1]);
    ...
    it("should timeout if clearing takes too long", async () => {
      ...
      await expect(
        mockContextService.clear("context-123", 0.25, 0.1)
      ).rejects.toThrow(APIError);
    });
