...
describe('TestFileSystem', () => {
  ...
  describe('test_read_file_success', () => {
    it('should read file successfully', async () => {
      ...
      const callMcpToolStub = sandbox.stub(mockFileSystem as any, 'callMcpTool')
        .resolves({
          data: {},
  ...
  describe('test_read_file_get_false', () => {
    it('should handle read file error', async () => {
      ...
      sandbox.stub(mockFileSystem as any, 'callMcpTool')
        .rejects(new Error('some error message'));

      await expect(mockFileSystem.readFile("/path/to/file.txt"))
  ...
  describe('test_create_directory_error', () => {
    it('should handle create directory error', async () => {
      ...
      await expect(mockFileSystem.createDirectory("/path/to/directory"))
  ...
  describe('test_edit_file_success', () => {
    it('should edit file successfully', async () => {
      ...
      const result = await mockFileSystem.editFile(
        "/path/to/file.txt",
  ...
  describe('test_edit_file_error', () => {
    it('should handle edit file error', async () => {
      ...
      await expect(mockFileSystem.editFile(
        "/path/to/file.txt",
        [{"oldText": "foo", "newText": "bar"}]
  ...
  describe('test_write_file_error', () => {
    it('should handle write file error', async () => {
      ...
      await expect(mockFileSystem.writeFile(
        "/path/to/file.txt",
        "content to write",
        "overwrite"
  ...
  describe('test_get_file_info_error', () => {
    it('should handle get file info error', async () => {
      ...
      await expect(mockFileSystem.getFileInfo("/path/to/file.txt"))
  ...
  describe('test_list_directory_error', () => {
    it('should handle list directory error', async () => {
      ...
      await expect(mockFileSystem.listDirectory("/path/to/directory"))
  ...
  describe('test_move_file_error', () => {
    it('should handle move file error', async () => {
      ...
      await expect(mockFileSystem.moveFile(
        "/path/to/source.txt",
        "/path/to/destination.txt"
  ...
  describe('test_read_multiple_files_success', () => {
    it('should read multiple files successfully', async () => {
      const callMcpToolStub = sandbox.stub(mockFileSystem as any, 'callMcpTool')
        .resolves({
          data: {},
          textContent: "/path/to/file1.txt: Content of file1\n\n---\n/path/to/file2.txt: \nContent of file2\n",
          isError: false,
          statusCode: 200,
          requestId: 'test-request-id'
        });
  ...
  describe('test_read_large_file_success', () => {
    it('should read large file successfully', async () => {
      ...
      const getFileInfoStub = sandbox.stub(mockFileSystem, 'getFileInfo')
        .resolves({
          data: { size: 150 * 1024, isDirectory: false, name: '', path: '', modTime: '', mode: '' },
          requestId: 'test-request-id'
        });

      // Mock readFile to return chunks of content
      const readFileStub = sandbox.stub(mockFileSystem, 'readFile')
        .onFirstCall().resolves({ data: "chunk1_content", requestId: 'test-request-id' })
        .onSecondCall().resolves({ data: "chunk2_content", requestId: 'test-request-id' })
        .onThirdCall().resolves({ data: "chunk3_content", requestId: 'test-request-id' });

      // Set a smaller chunk size for testing (50KB)
      const testChunkSize = 50 * 1024;

      const result = await mockFileSystem.readLargeFile("/path/to/large_file.txt", testChunkSize);
      ...
      expect(readFileStub.getCall(0).calledWith("/path/to/large_file.txt", 0, testChunkSize)).toBe(true);
      expect(readFileStub.getCall(1).calledWith("/path/to/large_file.txt", testChunkSize, testChunkSize)).toBe(true);
      expect(readFileStub.getCall(2).calledWith("/path/to/large_file.txt", testChunkSize * 2, testChunkSize)).toBe(true);
    });
  });
  ...
  describe('test_read_large_file_error', () => {
    it('should handle read large file error', async () => {
      ...
      sandbox.stub(mockFileSystem, 'getFileInfo')
        .rejects(new Error('File not found'));

      await expect(mockFileSystem.readLargeFile("/path/to/nonexistent_file.txt"))
        .rejects.toThrow('File not found');
    });
  });
  ...
  describe('test_write_large_file_success', () => {
    it('should write large file successfully', async () => {
      ...
      const writeFileStub = sandbox.stub(mockFileSystem, 'writeFile')
      ...
      expect(writeFileStub.getCall(0).calledWith(
        "/path/to/large_file.txt",
        largeContent.substring(0, testChunkSize),
        "overwrite"
      )).toBe(true);
      expect(writeFileStub.getCall(1).calledWith(
        "/path/to/large_file.txt",
        largeContent.substring(testChunkSize, testChunkSize * 2),
        "append"
      )).toBe(true);
      expect(writeFileStub.getCall(2).calledWith(
        "/path/to/large_file.txt",
  ...
  describe('test_write_large_file_small_content', () => {
    it('should write large file with small content', async () => {
      ...
      expect(writeFileStub.calledOnceWith(
        "/path/to/small_file.txt",
        smallContent,
        "overwrite"
      )).toBe(true);
  ...
  describe('test_write_large_file_error', () => {
    it('should handle write large file error', async () => {
      ...
      sandbox.stub(mockFileSystem, 'writeFile')
        .rejects(new Error('Failed to write large file'));

      // Create a large content string (150KB)
      const largeContent = "x".repeat(150 * 1024);

      await expect(mockFileSystem.writeLargeFile("/path/to/large_file.txt", largeContent))
