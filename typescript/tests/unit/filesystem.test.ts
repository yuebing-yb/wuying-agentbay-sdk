    ...
    it.only('should read a file', async () => {
      if (session.filesystem) {
        ...
        try {
          ...
          log(`ReadFile result: content='${readResponse.data.substring(0, 100)}...'`);
    ...
    it.only('should handle file not found errors', async () => {
      if (session.filesystem) {
        ...
        try {
          ...
          const readResponse = await session.filesystem.readFile(nonExistentFile);
          log(`ReadFile result for non-existent file: content='${readResponse.data}'`);
          log(`Read Non-existent File RequestId: ${readResponse.requestId || 'undefined'}`);
    ...
    it.only('should write to a file', async () => {
      ...
      if (session.filesystem && typeof session.filesystem.writeFile === 'function') {
        ...
        try {
          ...
          const writeResponse = await session.filesystem.writeFile(tempFile, content);
          ...
          log(`Read File after Write RequestId: ${readResponse.requestId || 'undefined'}`);
          ...
          if (session.command) {
            const commandResponse = await session.command.executeCommand(`rm ${tempFile}`);
            log(`Delete Temp File RequestId: ${commandResponse.requestId || 'undefined'}`);
    ...
    it.only('should create a directory', async () => {
      if (session.filesystem && typeof session.filesystem.createDirectory === 'function') {
        ...
        try {
          ...
          log(`Create Directory RequestId: ${createDirResponse.requestId || 'undefined'}`);
          ...
          if (session.command) {
            ...
            const lsResponse = await session.command.executeCommand(`ls -la ${testDirPath}`);
            ...
            if (session.command) {
              const rmdirResponse = await session.command.executeCommand(`rmdir ${testDirPath}`);
              log(`rmdir Command RequestId: ${rmdirResponse.requestId || 'undefined'}`);
    ...
    it.only('should edit a file by replacing text', async () => {
      if (session.filesystem && typeof session.filesystem.editFile === 'function') {
        ...
        try {
          ...
          const writeResponse = await session.filesystem.writeFile(testFilePath, initialContent);
          ...
          const editResponse = await session.filesystem.editFile(testFilePath, edits);
          ...
          log(`Read File after Edit RequestId: ${readResponse.requestId || 'undefined'}`);
          ...
          if (session.command) {
            const rmResponse = await session.command.executeCommand(`rm ${testFilePath}`);
  ...
  describe('listDirectory', () => {
    it.only('should list directory contents', async () => {
      if (session.filesystem && typeof session.filesystem.listDirectory === 'function') {
        ...
        try {
          const listResponse = await session.filesystem.listDirectory(`${TestPathPrefix}/`);
          log(`ListDirectory result: entries count=${listResponse.data.length}`);
          log(`List Directory RequestId: ${listResponse.requestId || 'undefined'}`);
    ...
    it.only('should move a file from source to destination', async () => {
      if (session.filesystem && typeof session.filesystem.moveFile === 'function') {
        ...
        try {
          ...
          const writeResponse = await session.filesystem.writeFile(sourceFilePath, testContent);
          log(`Created file for move test: ${sourceFilePath}`);
          log(`Write File RequestId: ${writeResponse.requestId || 'undefined'}`);

          // Move the file
          const moveResponse = await session.filesystem.moveFile(sourceFilePath, destFilePath);
          ...
          log(`Read File after Move RequestId: ${readResponse.requestId || 'undefined'}`);
          ...
          if (session.command) {
            const rmResponse = await session.command.executeCommand(`rm ${destFilePath}`);
            log(`rm Command RequestId: ${rmResponse.requestId || 'undefined'}`);
    ...
    it.only('should read multiple files at once', async () => {
      if (session.filesystem && typeof session.filesystem.readMultipleFiles === 'function') {
        ...
        try {
          ...
          const write1Response = await session.filesystem.writeFile(testFile1Path, file1Content);
          log(`Created test file 1: ${testFile1Path}`);
          log(`Write File 1 RequestId: ${write1Response.requestId || 'undefined'}`);

          const write2Response = await session.filesystem.writeFile(testFile2Path, file2Content);
          log(`Created test file 2: ${testFile2Path}`);
          log(`Write File 2 RequestId: ${write2Response.requestId || 'undefined'}`);
          ...
          if (session.command) {
            const rmResponse = await session.command.executeCommand(`rm ${testFile1Path} ${testFile2Path}`);
            log(`rm Command RequestId: ${rmResponse.requestId || 'undefined'}`);
    ...
    it.only('should search for files matching a pattern', async () => {
      if (session.filesystem && typeof session.filesystem.searchFiles === 'function') {
        ...
        try {
          ...
          const createDirResponse = await session.filesystem.createDirectory(testSubdirPath);
          ...
          const write1Response = await session.filesystem.writeFile(searchFile1Path, file1Content);
          const write2Response = await session.filesystem.writeFile(searchFile2Path, file2Content);
          const write3Response = await session.filesystem.writeFile(searchFile3Path, file3Content);
          log(`Created test files for search test`);

          // Search for files matching the pattern
          const searchResponse = await session.filesystem.searchFiles(testSubdirPath, searchPattern);
          log(`SearchFiles result: ${JSON.stringify(searchResponse.data)}`);
          log(`Search Files RequestId: ${searchResponse.requestId || 'undefined'}`);
          ...
          if (session.command) {
            const rmResponse = await session.command.executeCommand(`rm -rf ${testSubdirPath}`);
            log(`rm Command RequestId: ${rmResponse.requestId || 'undefined'}`);
