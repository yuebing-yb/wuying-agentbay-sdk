...
describe("MobileSimulateService", () => {
  ...
  describe("constructor", () => {
    test("should throw error if agentBay is not provided", () => {
      expect(() => new MobileSimulateService(null as any)).toThrow(
        "agentBay is required"
      );
    });

    test("should throw error if agentBay.context is not provided", () => {
      expect(() => new MobileSimulateService({} as any)).toThrow(
        "agentBay.context is required"
      );
    });
    ...
    test("should initialize with default values", () => {
      ...
      expect(mobileSimulateService.getSimulateMode()).toBe(
  ...
  describe("hasMobileInfo", () => {
    test("should throw error if contextSync is not provided", async () => {
      await expect(
        mobileSimulateService.hasMobileInfo(null as any)
      ).rejects.toThrow("contextSync is required");
    });

    test("should throw error if contextSync.contextId is not provided", async () => {
      const contextSync = new ContextSync("", "/test/path");
      await expect(
        mobileSimulateService.hasMobileInfo(contextSync)
      ).rejects.toThrow("contextSync.contextId is required");
    });
    ...
    test("should throw error if contextSync.path is not provided", async () => {
      ...
      await expect(
        mobileSimulateService.hasMobileInfo(contextSync)
      ).rejects.toThrow("contextSync.path is required");
    });
    ...
    test("should return true if mobile dev info file exists", async () => {
      ...
      mockContextService.listFiles.mockResolvedValue({
        success: true,
        entries: [{ fileName: "dev_info.json" }],
      });
    ...
    test("should return false if mobile dev info file does not exist", async () => {
      ...
      mockContextService.listFiles.mockResolvedValue({
        success: true,
        entries: [{ fileName: "other_file.json" }],
      });
  ...
  describe("uploadMobileInfo", () => {
    const validMobileInfo = JSON.stringify({
      model: "SM-A505F",
      brand: "Samsung",
    });

    test("should throw error if mobileDevInfoContent is not provided", async () => {
      await expect(mobileSimulateService.uploadMobileInfo("")).rejects.toThrow(
        "mobileDevInfoContent is required"
      );
    });

    test("should throw error if mobileDevInfoContent is not valid JSON", async () => {
      await expect(
        mobileSimulateService.uploadMobileInfo("invalid json")
      ).rejects.toThrow("mobileDevInfoContent is not a valid JSON string");
    });

    test("should throw error if mobileDevInfoContent is malformed JSON", async () => {
      await expect(
        mobileSimulateService.uploadMobileInfo('{"incomplete":')
      ).rejects.toThrow("mobileDevInfoContent is not a valid JSON string");
    });
    ...
    test("should create new context if not provided", async () => {
      ...
      const result = await mobileSimulateService.uploadMobileInfo(
        validMobileInfo
      );
    ...
    test("should throw error if contextSync is provided without contextId", async () => {
      ...
      await expect(
        mobileSimulateService.uploadMobileInfo(validMobileInfo, contextSync)
      ).rejects.toThrow("contextSync.contextId is required");
    });
    ...
    test("should upload with correct file path", async () => {
      ...
      await mobileSimulateService.uploadMobileInfo(
        validMobileInfo,
        contextSync
      );
